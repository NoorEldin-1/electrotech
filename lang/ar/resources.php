<?php

declare(strict_types=1);

return [

    /*
    |--------------------------------------------------------------------------
    | المشاريع - Project Resource
    |--------------------------------------------------------------------------
    */
    'projects' => [
        'label' => 'مشروع',
        'plural_label' => 'المشاريع',
        'navigation_label' => 'المشاريع',

        'sections' => [
            'project_information' => 'معلومات المشروع',
            'technical_specifications' => 'المواصفات الفنية',
            'financial_timeline' => 'البيانات المالية والجدول الزمني',
            'offers' => 'العروض',
            'offers_description' => 'أضف عرضاً مالياً / فنياً جديداً. آخر عرض هو الذي يظهر في قائمة المناقصات.',
            'offers_after_save_hint' => 'احفظ المشروع أولاً، ثم أضف العروض المفصّلة (جداول البنود والضريبة والشروط) من تبويب "العروض".',
            'attachments' => 'مرفقات المشروع',
            'attachments_description' => 'ارفع كل ملف تحت فئته المناسبة. الملفات المضغوطة (RAR / ZIP) لا تُفتح داخل المتصفح — نزّلها بزر التنزيل ثم افتحها. وتستطيع كل إدارة عرض وتنزيل ما يخصّها.',
            'description' => 'الوصف',
        ],

        'fields' => [
            'code' => 'كود المشروع',
            'name' => 'اسم المشروع / العملية',
            'client_name' => 'اسم العميل',
            'customer' => 'العميل',
            'consultant_name' => 'اسم الاستشاري',
            'engineer_name' => 'اسم المهندس',
            'electric_current' => 'التيار الكهربي',
            'model' => 'الطراز',
            'section_type' => 'اسم الموصل',
            'poles_count' => 'عدد الأقطاب',
            'quantity' => 'الكمية',
            'project_location' => 'مكان المشروع',
            'arrival_method' => 'طريقة ورود العملية',
            'status' => 'الحالة',
            'status_helper' => 'تبدأ العملية الجديدة في "المناقصات" فتظهر ضمن عمليات المناقصات. و"مسودة" تعني عملية محفوظة لم تُرسَل بعد. ثم تتحرك الحالة: مناقصة ← تحت اليد ← عملية نشطة عبر أزرار الإجراء / الإلغاء — ولا تُعدّل من هنا.',
            'estimated_budget' => 'الميزانية التقديرية',
            'actual_cost' => 'التكلفة الفعلية',
            'start_date' => 'تاريخ البداية',
            'end_date' => 'تاريخ النهاية',
            'end_date_helper' => 'يُضبط تلقائياً عند تحويل العملية إلى العمليات النشطة إذا تُرك فارغاً.',
            'alarm_at' => 'وقت التنبيه',
            'alarm_note' => 'ملاحظة التنبيه',
            'smb_status' => 'حالة SMB',
            'smb_received_at' => 'تاريخ استلام SMB',
            'acceptance_email_at' => 'تاريخ إيميل القبول',
            'manager_approved_at' => 'تاريخ موافقة المدير',
            'manager_approved_by' => 'موافقة المدير من',
            'lost_reason' => 'سبب الخسارة',
            'lost_reason_note' => 'ملاحظات الخسارة',
            'winning_competitor' => 'الشركة المنافسة الفائزة',
            'financial_amount' => 'المبلغ المالي',
            'submitted_at' => 'تاريخ التقديم',
            'offer_notes' => 'ملاحظات العرض',
            'latest_offer' => 'آخر عرض',
            'no_offers_yet' => 'لا توجد عروض مسجلة حتى الآن.',
            'description' => 'وصف المشروع / ملاحظات',
            'created_by' => 'أنشئ بواسطة',
        ],

        'columns' => [
            'code' => 'الكود',
            'name' => 'الاسم',
            'client_name' => 'العميل',
            'status' => 'الحالة',
            'estimated_budget' => 'الميزانية',
            'start_date' => 'تاريخ البداية',
            'created_by' => 'أنشئ بواسطة',
            'created_at' => 'تاريخ الإنشاء',
        ],

        'actions' => [
            'move_to_tender' => 'إرسال إلى المناقصات',
            'move_to_tender_modal_heading' => 'إرسال هذه العملية إلى المناقصات؟',
            'move_to_tender_modal_description' => 'بعد الإرسال ستظهر العملية في "عمليات المناقصات" بأزرار الإجراء / الإلغاء / التنبيه.',
            'change_status' => 'تغيير الحالة',
            'change_status_modal_heading' => 'تغيير حالة المشروع',
            'change_status_modal_description' => 'تجاوز إداري لمسار المبيعات الاعتيادي. يُسجَّل التغيير في سجل النشاط.',
        ],

        'notifications' => [
            'moved_to_tender' => 'تم إرسال العملية إلى المناقصات.',
            'move_to_tender_needs_offer_title' => 'لا يوجد عرض مسجل',
            'move_to_tender_needs_offer_body' => 'ادخل تعديل المشروع وأضف عرضاً واحداً على الأقل في قسم العروض قبل الإرسال للمناقصات.',
            'status_changed' => 'تم تحديث حالة المشروع.',
        ],

        'relations' => [
            'activities' => [
                'title' => 'السجل',
                'columns' => [
                    'date' => 'التوقيت',
                    'source' => 'المصدر',
                    'event' => 'الحدث',
                    'changes' => 'الحقول المتغيرة',
                    'causer' => 'بواسطة',
                ],
                'sources' => [
                    'project' => 'العملية',
                    'offer' => 'عرض سعر',
                    'payment' => 'دفعة',
                    'installation' => 'تركيب',
                    'delivery_voucher' => 'إذن تسليم',
                    'delivery_minute' => 'محضر تسليم',
                ],
                'events' => [
                    'created' => 'إنشاء',
                    'updated' => 'تعديل',
                    'deleted' => 'حذف',
                ],
            ],
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Project Offers — مستندات BOQ (شرائح 2، 7، 8)
    |--------------------------------------------------------------------------
    */
    'project_offers' => [
        'title' => 'العروض',
        'label' => 'عرض',
        'plural_label' => 'العروض',

        'sections' => [
            'header' => 'بيانات العرض',
            'groups' => 'جداول العرض',
            'items' => 'البنود',
        ],

        'fields' => [
            'quotation_number' => 'رقم العرض',
            'currency' => 'العملة',
            'vat_percentage' => 'نسبة ض.ق.م %',
            'show_vat' => 'إظهار الضريبة في العرض',
            'show_vat_helper' => 'عند إيقافه يكون إجمالي العرض بدون ضريبة القيمة المضافة.',
            'installation_percentage' => 'نسبة التركيب %',
            'show_installation' => 'إضافة التركيب إلى العرض',
            'show_installation_helper' => 'عند تفعيله يُضاف التركيب كنسبة من الإجمالي الفرعي (مثل الضريبة). بعض العملاء يطلبون التركيب كبند منفصل في العقد.',
            'installation_amount' => 'التركيب',
            'submitted_at' => 'تاريخ التقديم',
            'group_label' => 'عنوان الجدول',
            'conductor_type' => 'الموصل',
            'description' => 'البيان',
            'description_helper' => 'راجع البيان كاملاً قبل الطباعة — تأكد من الإملاء وعدد الأقطاب (3P / 4P) لأنه يغيّر السعر.',
            'unit' => 'وحدة القياس',
            'quantity' => 'الكمية',
            'unit_price' => 'سعر الوحدة',
            'line_total' => 'الإجمالي',
            'subtotal' => 'الإجمالي الفرعي',
            'tax_amount' => 'ض.ق.م',
            'grand_total' => 'الإجمالي النهائي',
            'totals' => 'الإجماليات',
            'terms' => 'ملاحظات / شروط',
            'terms_helper' => 'ملاحظات حرة تُكتب أسفل الجدول — شروط الدفع، مواعيد التسليم، نسبة التركيبات، وتفاصيل أخرى.',
            'special_terms' => 'الشروط الخاصة (خلف الجداول)',
            'special_terms_helper' => 'ملاحظات خاصة بالعرض تُطبع أسفل الجداول مباشرة — مثل «في حالة طلب التركيب تزداد الأسعار بواقع 15%».',
            'general_terms' => 'الشروط العامة (القياسية)',
            'general_terms_helper' => 'الشروط القياسية، نقطة في كل سطر. مملوءة مسبقاً من قالب؛ أضِف أو احذف النقاط حسب الحاجة. تُطبع كقائمة مرقّمة.',
            'header_note' => 'جملة الرأس (قبل الجداول)',
            'header_note_helper' => 'سطر تمهيدي + جملة ترخيص DKC، تُطبع قبل الجداول. مملوءة مسبقاً وقابلة للتعديل لكل عرض.',
        ],

        'defaults' => [
            'header_note' => "بالإشارة إلى طلب سيادتكم، نتشرف بتقديم عرضنا الخاص بمشروعكم طبقاً لجدول الأسعار التالي:\nThe busway system is manufactured under license from DKC – Italy.",
            'general_terms' => "مدة سريان العرض: أسبوع.\nحُسبت الأسعار على أساس سعر الدولار الحر طبقاً لموقع egcurrency.com، ويُحدَّد السعر النهائي عند السداد طبقاً لسعر الدولار وقت الدفع.\nالتوريد خلال 4-6 أسابيع من استلام أمر التوريد والدفعة المقدمة والرسومات التنفيذية المعتمدة.\nطريقة الدفع: دفعة مقدمة والباقي عند التسليم بمخازننا بالإسكندرية.\nتُحسب الإكسسوارات (وحدات النهاية والأكواع ووحدات الوصل) بسعر المتر الطولي طبقاً للطول الفعلي.\nجميع التوصيلات داخل اللوحة من مسؤولية مورّد اللوحات.\nالأسعار غير شاملة ضريبة القيمة المضافة.",
        ],

        'columns' => [
            'version' => 'الإصدار',
            'quotation_number' => 'رقم العرض',
            'grand_total' => 'الإجمالي النهائي',
            'is_winning' => 'الفائز',
            'submitted_at' => 'تاريخ التقديم',
        ],

        'actions' => [
            'add_group' => 'إضافة جدول',
            'add_item' => 'إضافة بند',
            'print' => 'طباعة',
            'print_en' => 'طباعة (إنجليزي)',
            'print_ar' => 'طباعة (عربي)',
        ],

        'pdf' => [
            'company_name' => 'إلكتروتك للصناعات الكهربائية',
            'quotation' => 'عرض سعر',
            'quotation_no' => 'رقم العرض',
            'date' => 'التاريخ',
            'project' => 'المشروع',
            'to' => 'إلى',
            'messrs' => 'السادة',
            'attention' => 'عناية السيد المهندس',
            'greeting' => 'تحية طيبة وبعد،،',
            'conductor_type' => 'نوع الموصل',
            'item_no' => 'م',
            'description' => 'البيان',
            'unit' => 'الوحدة',
            'qty' => 'الكمية',
            'unit_price' => 'سعر الوحدة',
            'total_price' => 'الإجمالي',
            'subtotal' => 'الإجمالي الفرعي',
            'taxes' => 'الضرائب',
            'installation' => 'التركيب',
            'grand_total' => 'الإجمالي النهائي',
            'offer_total_title' => 'إجمالي العرض',
            'special_terms_title' => 'الشروط الخاصة',
            'terms_title' => 'الشروط',
            'best_regards' => 'مع خالص التحية',
            'sales_manager' => 'مدير المبيعات',
        ],
    ],

    'sales_alerts' => [
        'column' => 'العرض',
        'missing_offer_tooltip' => 'لا يوجد عرض مُسعّر مسجل لهذه العملية بعد.',
        'notification_title' => 'عمليات بلا عرض',
        'notification_body' => 'يوجد :count عملية في المسار بلا عرض مُسعّر حتى الآن.',
        'offer_attached_title' => 'تم إرفاق عرض',
        'offer_attached_body' => 'العملية «:operation» أصبح لديها :count عرض/عروض مرفقة.',
        'submittal_title' => 'تم رفع Submittal',
        'submittal_body' => 'تم رفع Submittal للعملية «:operation» (:count ملف).',
        'view_operation' => 'فتح العملية',
        'missing_offer_alert_title' => 'عملية مناقصة بلا عرض',
        'missing_offer_alert_body' => 'العملية «:operation» ليس لها عرض مُسعّر بعد. أضِف عرضًا.',
        'missing_smb_alert_title' => 'عملية تحت اليد بلا SMB',
        'missing_smb_alert_body' => 'العملية «:operation» ليس لها SMB مُسجّل بعد. ارفع ملف الـ SMB.',
    ],

    /*
    |--------------------------------------------------------------------------
    | عمليات المناقصات (الشريحة 5)
    |--------------------------------------------------------------------------
    */
    'tender_projects' => [
        'label' => 'عملية مناقصة',
        'plural_label' => 'عمليات المناقصات',
        'navigation_label' => 'عمليات المناقصات',

        'columns' => [
            'name' => 'اسم العملية',
            'financial_offer' => 'العرض المالي',
            'alarm' => 'تنبيه',
            'date' => 'التاريخ',
        ],

        'fields' => [
            'smb_file' => 'ملف SMB / Submittal',
            'smb_file_helper' => 'ارفع مستند SMB / Submittal (الملف نفسه وليس ملاحظة). اختياري — يمكن إضافته لاحقًا من قسم Submittal الخاص بالمشروع.',
            'lost_reason' => 'سبب الخسارة',
            'lost_reason_note' => 'ملاحظات الخسارة',
            'winning_competitor' => 'الشركة المنافسة الفائزة',
            'alarm_at' => 'وقت التنبيه',
            'alarm_note' => 'ملاحظة التنبيه',
        ],

        'actions' => [
            'action' => 'إجراء — نقل إلى تحت اليد',
            'action_modal_heading' => 'نقل هذه العملية إلى تحت اليد؟',
            'action_modal_description' => 'العميل قبل وطلب تجهيز SMB. ارفع ملف SMB / Submittal (اختياري — يمكنك إضافته لاحقًا).',
            'cancel' => 'إلغاء — نقل إلى المفقودة',
            'cancel_modal_heading' => 'إلغاء العملية ونقلها إلى المفقودة؟',
            'set_alarm' => 'ضبط تنبيه',
            'clear_alarm' => 'مسح التنبيه',
        ],

        'notifications' => [
            'moved_to_inhand' => 'تم نقل العملية إلى تحت اليد.',
            'moved_to_lost' => 'تم نقل العملية إلى المفقودة.',
            'alarm_set' => 'تم ضبط التنبيه.',
            'alarm_cleared' => 'تم مسح التنبيه.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | العمليات تحت اليد (الشريحة 7)
    |--------------------------------------------------------------------------
    */
    'in_hand_projects' => [
        'label' => 'عملية تحت اليد',
        'plural_label' => 'العمليات تحت اليد',
        'navigation_label' => 'العمليات تحت اليد',

        'columns' => [
            'name' => 'اسم العملية',
            'smb' => 'SMB',
            'acceptance_email_at' => 'إيميل القبول',
            'manager_approved' => 'موافقة المدير',
            'alarm' => 'تنبيه',
            'date' => 'التاريخ',
        ],

        'smb_present_tooltip' => 'ملف SMB / Submittal مرفوع.',
        'smb_missing_tooltip' => 'لا يوجد ملف SMB / Submittal بعد — ارفعه من قسم Submittal الخاص بالمشروع.',

        'fields' => [
            'acceptance_email_at' => 'تاريخ إيميل القبول',
            'acceptance_file' => 'ملف قبول العميل',
            'acceptance_file_helper' => 'أرفِق مستند قبول العميل (الملف نفسه، وليس مجرد ملاحظة).',
            'manager_approve_now' => 'موافقة المدير الآن',
            'manager_approve_helper' => 'يظهر فقط للمستخدمين الذين لديهم صلاحية موافقة المدير.',
            'lost_reason' => 'سبب الخسارة',
            'lost_reason_note' => 'ملاحظات الخسارة',
            'winning_competitor' => 'الشركة المنافسة الفائزة',
            'alarm_at' => 'وقت التنبيه',
            'alarm_note' => 'ملاحظة التنبيه',
        ],

        'actions' => [
            'action' => 'إجراء — نقل إلى العمليات النشطة',
            'action_modal_heading' => 'نقل هذه العملية إلى العمليات النشطة؟',
            'action_modal_description' => 'يتطلب تاريخ إيميل القبول وموافقة المدير.',
            'manager_approve' => 'موافقة المدير',
            'manager_approve_modal_heading' => 'اعتماد هذه العملية كمدير؟',
            'cancel' => 'إلغاء — نقل إلى المفقودة',
            'cancel_modal_heading' => 'إلغاء العملية ونقلها إلى المفقودة؟',
            'set_alarm' => 'ضبط تنبيه',
            'clear_alarm' => 'مسح التنبيه',
        ],

        'notifications' => [
            'moved_to_active' => 'تم نقل العملية إلى العمليات النشطة.',
            'moved_to_lost' => 'تم نقل العملية إلى المفقودة.',
            'manager_approved' => 'تم تسجيل موافقة المدير.',
            'alarm_set' => 'تم ضبط التنبيه.',
            'alarm_cleared' => 'تم مسح التنبيه.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | العمليات النشطة
    |--------------------------------------------------------------------------
    */
    'active_projects' => [
        'label' => 'عملية نشطة',
        'plural_label' => 'العمليات النشطة',
        'navigation_label' => 'العمليات النشطة',

        'columns' => [
            'code' => 'الكود',
            'name' => 'اسم العملية',
            'client_name' => 'العميل',
            'actual_cost' => 'التكلفة الفعلية',
            'start_date' => 'تاريخ البداية',
            'winning_offer' => 'العرض الفائز',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | العمليات المفقودة (الشريحة 4)
    |--------------------------------------------------------------------------
    */
    'lost_projects' => [
        'label' => 'عملية مفقودة',
        'plural_label' => 'العمليات المفقودة',
        'navigation_label' => 'قائمة المفقودة',

        'columns' => [
            'name' => 'اسم العملية',
            'client_name' => 'العميل',
            'lost_reason' => 'سبب الخسارة',
            'winning_competitor' => 'الشركة المنافسة الفائزة',
            'date' => 'التاريخ',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | الأصناف - Item Resource
    |--------------------------------------------------------------------------
    */
    'items' => [
        'label' => 'صنف',
        'plural_label' => 'الأصناف',
        'navigation_label' => 'الأصناف',

        'sections' => [
            'item_details' => 'تفاصيل الصنف',
        ],

        'fields' => [
            'sku' => 'الرقم المرجعي / رقم القطعة',
            'name' => 'الاسم',
            'type' => 'النوع',
            'unit' => 'وحدة القياس',
            'unit_cost' => 'تكلفة الوحدة',
            'minimum_stock' => 'الحد الأدنى للمخزون',
            'description' => 'الوصف',
        ],

        'columns' => [
            'sku' => 'الرقم المرجعي',
            'name' => 'الاسم',
            'type' => 'النوع',
            'unit' => 'الوحدة',
            'unit_cost' => 'تكلفة الوحدة',
            'on_hand' => 'المتاح',
            'on_hold' => 'المحجوز',
            'minimum_stock' => 'الحد الأدنى',
        ],

        'actions' => [
            'quick_view' => 'عرض الصنف',
            'close' => 'إغلاق',
        ],

        'modal' => [
            'stock' => 'المخزون',
            'on_hand' => 'الرصيد الفعلي',
            'on_hold' => 'المحجوز',
            'available' => 'المتاح',
            'value' => 'القيمة',
            'scrap' => 'الفضلات',
            'scrap_on_hand' => 'رصيد الفضلات',
            'scrap_value' => 'قيمة الفضلات',
            'below_minimum' => 'الرصيد المتاح أقل من الحد الأدنى.',
            'not_found' => 'الصنف غير موجود.',
        ],

        // لاحقة تُضاف لاسم صنف الفضلات المُنشأ تلقائياً.
        'scrap_label' => 'فضلات',
    ],

    /*
    |--------------------------------------------------------------------------
    | قوائم المواد - BOM Resource
    |--------------------------------------------------------------------------
    */
    'boms' => [
        'label' => 'قائمة مواد',
        'plural_label' => 'قوائم المواد',
        'navigation_label' => 'قوائم المواد',

        'sections' => [
            'bom_details' => 'تفاصيل قائمة المواد',
            'bom_details_description' => 'اربط القائمة بمشروع، أو بمنتج تام لتصبح التركيبة القياسية لهذا المنتج.',
            'bom_items' => 'أصناف قائمة المواد',
            'bom_items_description' => 'أضف الأصناف المطلوبة لهذا المشروع. أدخل نسبة الهالك وفقاً للمواصفات الفنية.',
        ],

        'fields' => [
            'project' => 'المشروع',
            'output_item' => 'المنتج التام (قائمة قياسية)',
            'output_item_helper' => 'حدّده لتصبح القائمة هي التركيبة القياسية للمنتج، تُجلب تلقائياً في أوامر التصنيع.',
            'version' => 'الإصدار',
            'status' => 'الحالة',
            'notes' => 'ملاحظات',
            'item' => 'الصنف',
            'quantity' => 'الكمية',
            'waste_percentage' => 'نسبة الهالك %',
        ],

        'columns' => [
            'project' => 'المشروع',
            'standard_bom' => 'قائمة قياسية',
            'output_item' => 'المنتج التام',
            'sales_stage' => 'مرحلة المبيعات',
            'version' => 'الإصدار',
            'status' => 'الحالة',
            'items_count' => 'الأصناف',
            'prepared_by' => 'أعدّ بواسطة',
            'approved_by' => 'اعتُمد بواسطة',
            'created_at' => 'تاريخ الإنشاء',
        ],

        'actions' => [
            'approve' => 'اعتماد',
            'add_bom_item' => 'إضافة صنف',
        ],

        'notifications' => [
            'approved' => 'تم اعتماد قائمة المواد بنجاح',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | أوامر الشراء - Purchase Order Resource
    |--------------------------------------------------------------------------
    */
    'purchase_orders' => [
        'label' => 'أمر شراء',
        'plural_label' => 'أوامر الشراء',
        'navigation_label' => 'أوامر الشراء',

        'sections' => [
            'po_details' => 'تفاصيل أمر الشراء',
            'line_items' => 'بنود الأمر',
            'totals' => 'الإجماليات',
            'attachment' => 'صورة أمر الشراء',
        ],

        'fields' => [
            'po_number' => 'رقم أمر الشراء',
            'project' => 'المشروع',
            'project_placeholder' => 'بدون مشروع — شراء للمخازن',
            'project_help' => 'اتركه فارغًا لأمر شراء خاص بالمخازن/المخزون غير مرتبط بأي عملية.',
            'supplier' => 'المورد',
            'supplier_name' => 'اسم المورد',
            'supplier_contact' => 'بيانات تواصل المورد',
            'status' => 'الحالة',
            'subtotal' => 'الإجمالي الفرعي',
            'vat_amount' => 'ض. القيمة المضافة (:rate%)',
            'profit_tax_amount' => 'خصم ض. الأرباح (:rate%)',
            'total_amount' => 'الإجمالي',
            'expected_delivery_date' => 'تاريخ التسليم المتوقع',
            'notes' => 'ملاحظات',
            'item' => 'الصنف',
            'quantity' => 'الكمية',
            'unit_price' => 'سعر الوحدة (ج.م)',
            'received_quantity' => 'الكمية المستلمة',
        ],

        'columns' => [
            'po_number' => 'رقم الأمر',
            'project' => 'المشروع',
            'warehouse_po' => 'المخازن',
            'sales_stage' => 'مرحلة المبيعات',
            'supplier_name' => 'المورد',
            'status' => 'الحالة',
            'total_amount' => 'المبلغ الإجمالي',
            'expected_delivery' => 'التسليم المتوقع',
            'created_at' => 'تاريخ الإنشاء',
        ],

        'actions' => [
            'receive' => 'استلام الأصناف',
            'receive_hint' => 'أدخل الكميات المستلمة. يُنشأ إذن إضافة ويُرحَّل تلقائيًا — يُحدَّث المخزون وحساب المورد ويُقفَل أمر الشراء بالمقارنة.',
            'add_item' => 'إضافة صنف',
            'approve' => 'اعتماد',
            'approve_confirm' => 'اعتماد أمر الشراء؟ سيتحوّل إلى «مُرسَل» ولا يمكن تعديله كمسودة بعد ذلك.',
            'open_item' => 'فتح كرت الصنف',
            'print' => 'طباعة',
            'print_en' => 'طباعة (إنجليزي)',
            'print_ar' => 'طباعة (عربي)',
        ],

        'notifications' => [
            'received' => 'تم استلام الأصناف بنجاح',
            'receive_failed' => 'فشل في الاستلام',
            'no_quantities' => 'لم يتم إدخال كميات',
            'approved' => 'تم اعتماد أمر الشراء وإرساله',
            'voucher_created' => 'تم إنشاء وترحيل إذن الإضافة :number.',
        ],

        'pdf' => [
            'company_name' => 'إلكتروتك للصناعات الكهربائية',
            'title' => 'أمر شراء',
            'po_number' => 'رقم الأمر',
            'date' => 'التاريخ',
            'supplier' => 'المورد',
            'tax_number' => 'الرقم الضريبي',
            'project' => 'المشروع',
            'warehouse' => 'المخازن (شراء للمخزون)',
            'status' => 'الحالة',
            'item_no' => 'م',
            'item' => 'الصنف',
            'qty' => 'الكمية',
            'unit_price' => 'سعر الوحدة',
            'total_price' => 'الإجمالي',
            'subtotal' => 'الإجمالي الفرعي',
            'vat' => 'ض. القيمة المضافة (:rate%)',
            'profit_tax' => 'خصم ض. الأرباح (:rate%)',
            'total' => 'الإجمالي',
            'no_profit_tax_note' => 'هذا المورد غير خاضع لخصم 1% ضريبة الأرباح التجارية والصناعية.',
            'approved_by' => 'اعتماد مدير مكتب ادارة المشروعات',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | حركات المخزون - Inventory Transaction Resource
    |--------------------------------------------------------------------------
    */
    'inventory_transactions' => [
        'label' => 'حركة مخزون',
        'plural_label' => 'حركات المخزون',
        'navigation_label' => 'حركات المخزون',

        'columns' => [
            'date' => 'التاريخ',
            'item' => 'الصنف',
            'sku' => 'الرقم المرجعي',
            'type' => 'النوع',
            'warehouse' => 'المخزن',
            'quantity' => 'الكمية',
            'unit_cost' => 'تكلفة الوحدة',
            'source' => 'المصدر',
            'notes' => 'ملاحظات',
            'performed_by' => 'بواسطة',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Stock Card (Moving Weighted Average)
    |--------------------------------------------------------------------------
    */
    'stock_card' => [
        'heading' => 'كرت الصنف',
        'subheading' => 'تقييم بطريقة المتوسط المرجح المتحرك',
        'warehouse' => 'المخزن',
        'empty' => 'لا توجد حركات مخزون مسجّلة لهذا المخزن حتى الآن.',
        'groups' => [
            'incoming' => 'وارد',
            'outgoing' => 'صادر',
            'balance' => 'الرصيد',
        ],
        'columns' => [
            'date' => 'التاريخ',
            'statement' => 'بيان',
            'quantity' => 'كمية',
            'price' => 'سعر',
            'value' => 'رصيد',
        ],
        'total' => 'الإجمالي',
        'documents' => [
            'AdditionVoucher' => 'إذن إضافة',
            'IssueVoucher' => 'إذن صرف',
            'ReturnVoucher' => 'إذن ارتداد',
            'DeliveryVoucher' => 'إذن تسليم',
            'PurchaseOrder' => 'أمر شراء',
            'WorkOrder' => 'أمر تشغيل',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | الموردون - Supplier Resource
    |--------------------------------------------------------------------------
    */
    'suppliers' => [
        'label' => 'مورد',
        'plural_label' => 'الموردون',
        'navigation_label' => 'الموردون',

        'sections' => [
            'details' => 'بيانات المورد',
            'documents' => 'المستندات',
        ],

        'fields' => [
            'name' => 'اسم المورد',
            'contact_person' => 'مسؤول التواصل',
            'phone' => 'الهاتف',
            'email' => 'البريد الإلكتروني',
            'tax_number' => 'الرقم الضريبي',
            'profit_tax_exempt' => 'معفى من خصم 1% ضريبة الأرباح',
            'profit_tax_exempt_helper' => 'عند التفعيل لا يُخصم 1% ضريبة الأرباح التجارية والصناعية على أوامر شراء هذا المورد. أرفِق مستند الإعفاء بالأسفل.',
            'address' => 'العنوان',
            'notes' => 'ملاحظات',
        ],

        'columns' => [
            'name' => 'الاسم',
            'contact_person' => 'جهة التواصل',
            'phone' => 'الهاتف',
            'email' => 'البريد الإلكتروني',
            'profit_tax_exempt' => 'إعفاء 1%',
            'balance' => 'الرصيد',
            'balance_nature' => 'طبيعة الرصيد',
            'created_at' => 'تاريخ الإنشاء',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | العملاء - Customer Resource
    |--------------------------------------------------------------------------
    */
    'customers' => [
        'label' => 'عميل',
        'plural_label' => 'العملاء',
        'navigation_label' => 'العملاء',

        'sections' => [
            'details' => 'بيانات العميل',
            'attachments' => 'المرفقات',
        ],

        'fields' => [
            'name' => 'اسم العميل',
            'contact_person' => 'مسؤول التواصل',
            'phone' => 'الهاتف',
            'email' => 'البريد الإلكتروني',
            'tax_number' => 'الرقم الضريبي',
            'address' => 'العنوان',
            'notes' => 'ملاحظات',
        ],

        'columns' => [
            'name' => 'الاسم',
            'contact_person' => 'جهة التواصل',
            'phone' => 'الهاتف',
            'email' => 'البريد الإلكتروني',
            'balance' => 'الرصيد',
            'balance_nature' => 'طبيعة الرصيد',
            'created_at' => 'تاريخ الإنشاء',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | إذن الإضافة - Addition Voucher Resource
    |--------------------------------------------------------------------------
    */
    'addition_vouchers' => [
        'label' => 'إذن إضافة',
        'plural_label' => 'أذون الإضافة',
        'navigation_label' => 'أذون الإضافة',

        'sections' => [
            'details' => 'بيانات الإذن',
            'lines' => 'الأصناف الواردة',
            'documents' => 'صورة الإذن',
        ],

        'fields' => [
            'voucher_number' => 'رقم الإذن',
            'supplier' => 'المورد',
            'supplier_name' => 'اسم المورد (غير مسجّل)',
            'purchase_order' => 'طلب الشراء',
            'voucher_date' => 'التاريخ',
            'invoice_number' => 'رقم الفاتورة',
            'invoice_number_hint' => 'يُترك فارغاً حتى تصل فاتورة المورد.',
            'invoice_date' => 'تاريخ الفاتورة',
            'invoice_value' => 'قيمة الفاتورة',
            'invoice_value_posted_hint' => 'الإذن مُرحَّل — تعديل القيمة يصحّح قيد المورد المرحَّل.',
            'closure_reason' => 'سبب الإقفال',
            'closure_reason_hint' => 'لماذا لن تصل فاتورة لهذا الإذن؟ (مثل: بديل مجاني، عينة، مرتجع ضمان)',
            'notes' => 'ملاحظات',
            'lines' => 'الأصناف',
            'item' => 'الصنف',
            'quantity' => 'الكمية',
            'unit_cost' => 'تكلفة الوحدة',
        ],

        'columns' => [
            'voucher_number' => 'الرقم',
            'supplier' => 'المورد',
            'voucher_date' => 'التاريخ',
            'invoice_number' => 'الفاتورة',
            'invoice_value' => 'قيمة الفاتورة',
            'received_value' => 'قيمة الوارد',
            'invoicing_status' => 'حالة الفوترة',
            'total_received' => 'إجمالي الوارد',
            'total_invoiced' => 'إجمالي المفوتر',
            'value_mismatch' => 'فرق عن الوارد: :difference جنيه',
            'status' => 'الحالة',
        ],

        'filters' => [
            'value_mismatch' => 'الفاتورة ≠ قيمة الوارد',
        ],

        'actions' => [
            'post' => 'ترحيل',
            'post_confirm' => 'الترحيل يضيف الأصناف للمخزون ويُرحّل القيمة لحساب المورد. لا يمكن التراجع.',
            'record_invoice' => 'تسجيل فاتورة المورد',
            'close_without_invoice' => 'إقفال بدون فاتورة',
            'reopen_invoicing' => 'إعادة الفتح للفوترة',
            'reopen_invoicing_confirm' => 'يعود الإذن لحالة انتظار فاتورة المورد ويُمسح سبب الإقفال.',
        ],

        'notifications' => [
            'posted' => 'تم ترحيل إذن الإضافة — تم تحديث المخزون وحساب المورد.',
            'invoice_recorded' => 'تم تسجيل فاتورة المورد على الإذن.',
            'closed_uninvoiced' => 'تم إقفال الإذن بدون فاتورة — وحُفظ السبب.',
            'invoicing_reopened' => 'تمت إعادة فتح الإذن — في انتظار فاتورة المورد من جديد.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | إذن الصرف - Issue Voucher Resource
    |--------------------------------------------------------------------------
    */
    'issue_vouchers' => [
        'label' => 'إذن صرف',
        'plural_label' => 'أذون الصرف',
        'navigation_label' => 'أذون الصرف',

        'sections' => [
            'details' => 'بيانات الإذن',
            'lines' => 'الأصناف المنصرفة',
            'lines_description' => 'تُملأ تلقائياً من خامات أمر التصنيع بالكميات المتبقية وأسعارها عند اختيار الأمر — وتقدر تعدّلها بعد الملء.',
        ],

        'fields' => [
            'voucher_number' => 'رقم الإذن',
            'work_order' => 'أمر التصنيع',
            'work_order_helper' => 'اختيار الأمر يملأ الأصناف المنصرفة تلقائياً بالكميات المتبقية من خطة خاماته.',
            'voucher_date' => 'التاريخ',
            'notes' => 'ملاحظات',
            'lines' => 'الأصناف',
            'item' => 'الصنف',
            'quantity' => 'الكمية',
            'unit_cost' => 'تكلفة الوحدة',
            'remaining_required' => 'المتبقّي المطلوب لأمر التصنيع: :qty',
            'not_in_plan' => '⚠ هذا الصنف غير موجود في خامات أمر التصنيع.',
            'excess_reason' => 'سبب الصرف الزائد',
            'excess_reason_helper' => 'يُسجَّل على الإذن مع اسمك وتاريخ الاعتماد.',
        ],

        'columns' => [
            'voucher_number' => 'الرقم',
            'work_order' => 'أمر التصنيع',
            'voucher_date' => 'التاريخ',
            'total_value' => 'إجمالي القيمة',
            'status' => 'الحالة',
            'has_excess' => 'صرف زائد',
        ],

        'actions' => [
            'post' => 'ترحيل',
            'post_confirm' => 'الترحيل ينقل الخامات من المخزن إلى تحت التشغيل ويُحمّل قيمتها على العملية. لا يمكن التراجع.',
            'refill' => 'إعادة الجلب من أمر التصنيع',
            'refill_confirm' => 'سيتم استبدال كل الأصناف الحالية بالكميات المتبقية من خطة خامات أمر التصنيع.',
        ],

        'excess' => [
            'intro' => 'الكميات التالية تتجاوز ما يحتاجه أمر التصنيع فعلياً بعد خصم ما سبق صرفه (وبعد رد المرتجعات).',
            'may_approve' => 'تقدر ترجع وتعدّل الكميات، أو تستكمل الترحيل باعتماد الزيادة وكتابة سببها بالأسفل.',
            'may_not_approve' => 'ليست لديك صلاحية اعتماد الصرف الزائد — ارجع وعدّل الكميات، أو اطلب اعتمادها من صاحب الصلاحية.',
            'live_warning' => '⚠ كمية زائدة عن حاجة أمر التصنيع: :items',
            'columns' => [
                'item' => 'الصنف',
                'required' => 'المطلوب بالخطة',
                'previously_issued' => 'سبق صرفه',
                'remaining' => 'المتبقّي',
                'this_voucher' => 'في هذا الإذن',
                'excess' => 'الزيادة',
            ],
        ],

        'notifications' => [
            'posted' => 'تم ترحيل إذن الصرف — تم نقل الخامات إلى تحت التشغيل.',
            'lines_filled' => 'تم ملء الأصناف من أمر التصنيع',
            'lines_filled_body' => 'تم جلب :count صنف بالكميات المتبقية وأسعارها. تقدر تعدّلها.',
            'nothing_remaining' => 'لا توجد كميات متبقية',
            'nothing_remaining_body' => 'كل خامات أمر التصنيع تم صرفها بالفعل. أضف الأصناف يدوياً لو فيه صرف إضافي.',
            'excess_blocked' => 'لا يمكن الترحيل — كمية زائدة عن حاجة أمر التصنيع',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | Return Voucher Resource (إذن ارتداد)
    |--------------------------------------------------------------------------
    */
    'return_vouchers' => [
        'label' => 'إذن ارتداد',
        'plural_label' => 'أذون الارتداد',
        'navigation_label' => 'أذون الارتداد',

        'sections' => [
            'details' => 'بيانات الإذن',
            'lines' => 'الأصناف المرتدّة',
        ],

        'fields' => [
            'voucher_number' => 'رقم الإذن',
            'work_order' => 'أمر التصنيع',
            'voucher_date' => 'التاريخ',
            'notes' => 'ملاحظات',
            'lines' => 'الأصناف',
            'item' => 'الصنف',
            'quantity' => 'الكمية',
            'unit_cost' => 'تكلفة الوحدة',
        ],

        'columns' => [
            'voucher_number' => 'الرقم',
            'work_order' => 'أمر التصنيع',
            'voucher_date' => 'التاريخ',
            'total_value' => 'إجمالي القيمة',
            'status' => 'الحالة',
        ],

        'actions' => [
            'post' => 'ترحيل',
            'post_confirm' => 'الترحيل يُرجِع الفضلات من تحت التشغيل إلى مخزن الخامات بكود مختلف ويعكس قيمتها عن العملية. لا يمكن التراجع.',
        ],

        'notifications' => [
            'posted' => 'تم ترحيل إذن الارتداد — تم إرجاع الفضلات إلى مخزن الخامات.',
        ],
    ],

    'depreciation_vouchers' => [
        'label' => 'إذن إهلاك',
        'plural_label' => 'أذون الإهلاك',
        'navigation_label' => 'أذون الإهلاك',

        'sections' => [
            'details' => 'بيانات الإذن',
            'lines' => 'الأصناف الهالكة',
        ],

        'fields' => [
            'voucher_number' => 'رقم الإذن',
            'work_order' => 'أمر التصنيع',
            'loss_type' => 'نوع الهالك',
            'loss_type_hint' => 'الهالك الغير طبيعي يُخرَج من العملية، أما الطبيعي فيبقى محمَّلاً عليها.',
            'voucher_date' => 'التاريخ',
            'notes' => 'ملاحظات',
            'lines' => 'الأصناف',
            'item' => 'الصنف',
            'quantity' => 'الكمية',
            'unit_cost' => 'تكلفة الوحدة',
        ],

        'columns' => [
            'voucher_number' => 'الرقم',
            'work_order' => 'أمر التصنيع',
            'loss_type' => 'نوع الهالك',
            'voucher_date' => 'التاريخ',
            'total_value' => 'إجمالي القيمة',
            'status' => 'الحالة',
        ],

        'actions' => [
            'post' => 'ترحيل',
            'post_confirm' => 'الترحيل يُخرج الهالك من تحت التشغيل ويخفّض قيمة الصنف على كرت الصنف ويُرحّله إلى حساب الهالك بقيد محاسبي. والهالك الغير طبيعي يُعكَس أيضاً عن العملية. لا يمكن التراجع.',
        ],

        'notifications' => [
            'posted' => 'تم ترحيل إذن الإهلاك — تم ترحيل الهالك إلى حساب الهالك.',
        ],

        'journal_description' => 'إهلاك هالك تصنيع :number (أمر تشغيل #:wo)',
    ],

    /*
    |--------------------------------------------------------------------------
    | كشف الحساب - Account Entry Resource
    |--------------------------------------------------------------------------
    */
    'account_entries' => [
        'label' => 'قيد حساب',
        'plural_label' => 'دفتر الحسابات',
        'navigation_label' => 'دفتر الحسابات',
        'statement' => 'كشف الحساب',

        'columns' => [
            'date' => 'التاريخ',
            'party' => 'الحساب',
            'reference' => 'المرجع',
            'operation' => 'العملية',
            'direction' => 'النوع',
            'amount' => 'القيمة',
            'running_balance' => 'الرصيد',
        ],

        'filters' => [
            'from' => 'من',
            'until' => 'إلى',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | شجرة الحسابات - Chart of Accounts Resource
    |--------------------------------------------------------------------------
    */
    'accounts' => [
        'label' => 'حساب',
        'plural_label' => 'شجرة الحسابات',
        'navigation_label' => 'شجرة الحسابات',

        'sections' => [
            'details' => 'بيانات الحساب',
            'opening' => 'الرصيد الافتتاحي',
            'statements' => 'موضع الحساب في القوائم المالية',
            'statements_hint' => 'يحدد أين يظهر هذا الحساب في قائمة التشغيل والدخل والمركز المالي والتدفقات النقدية. لا يؤثر على الترحيل ولا على ميزان المراجعة.',
        ],

        'fields' => [
            'code' => 'الكود',
            'name' => 'الاسم',
            'name_en' => 'الاسم (إنجليزي)',
            'type' => 'النوع',
            'nature' => 'الطبيعة',
            'currency' => 'العملة',
            'parent' => 'الحساب الأب',
            'is_active' => 'نشط',
            'opening_balance' => 'الرصيد الافتتاحي',
            'opening_balance_hint' => 'موجب/سالب حسب طبيعة الحساب (رصيد أول المدة).',
            'opening_balance_date' => 'تاريخ الرصيد الافتتاحي',
            'notes' => 'ملاحظات',
            'statement_section' => 'بند القوائم المالية',
            'statement_section_hint' => 'لو تُرك فارغاً سيظهر الحساب تلقائياً في بند «:default» المشتق من نوعه.',
            'statement_section_hint_generic' => 'لو تُرك فارغاً سيُشتق البند تلقائياً من نوع الحساب.',
            'contra_of' => 'مجمع إهلاك لحساب',
            'contra_of_hint' => 'يُخصم رصيد هذا الحساب من تكلفة الأصل المختار في عمود مستقل بقائمة المركز المالي.',
        ],

        'columns' => [
            'code' => 'الكود',
            'name' => 'الحساب',
            'type' => 'النوع',
            'nature' => 'الطبيعة',
            'currency' => 'العملة',
            'opening_balance' => 'الرصيد الافتتاحي',
            'is_active' => 'نشط',
            'statement_section' => 'بند القوائم المالية',
            'statement_section_inherited' => 'غير مصنَّف — البند مشتق تلقائياً من نوع الحساب.',
        ],

        'filters' => [
            'unclassified' => 'حسابات غير مصنَّفة على بند',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | قيود اليومية - Journal Entry Resource
    |--------------------------------------------------------------------------
    */
    'journal_entries' => [
        'label' => 'قيد يومية',
        'plural_label' => 'قيود اليومية',
        'navigation_label' => 'قيود اليومية',

        'sections' => [
            'details' => 'بيانات القيد',
            'lines' => 'سطور القيد',
            'debit_lines' => 'مدين',
            'credit_lines' => 'دائن',
        ],

        'fields' => [
            'entry_serial' => 'رقم القيد',
            'entry_number' => 'مرجع القيد',
            'document_type' => 'نوع المستند',
            'document_number' => 'رقم المستند',
            'entry_date' => 'التاريخ',
            'description' => 'البيان',
            'currency' => 'العملة',
            'notes' => 'ملاحظات',
            'lines' => 'السطور',
            'account' => 'الحساب',
            'project' => 'العملية (مركز التكلفة)',
            'direction' => 'النوع',
            'amount' => 'القيمة',
            'line_notes' => 'ملاحظات',
            'totals' => 'الإجماليات',
            'show_line_details' => 'إظهار مركز التكلفة وملاحظات السطر',
        ],

        'columns' => [
            'entry_serial' => 'رقم القيد',
            'entry_number' => 'مرجع القيد',
            'document_type' => 'المستند',
            'document_number' => 'رقم المستند',
            'entry_date' => 'التاريخ',
            'description' => 'البيان',
            'total_debit' => 'مدين',
            'total_credit' => 'دائن',
            'status' => 'الحالة',
        ],

        'filters' => [
            'from' => 'من',
            'until' => 'إلى',
        ],

        'helpers' => [
            'document_number' => 'اتركه فارغاً ليأخذ الرقم التالي في تسلسل نوع المستند.',
            'show_line_details' => 'اتركه مقفولاً للإدخال السريع — الحساب والقيمة فقط.',
        ],

        'actions' => [
            'create' => 'إضافة قيد يومية',
            'post' => 'ترحيل',
            'post_confirm' => 'الترحيل يعكس القيد في دفاتر الأستاذ وميزان المراجعة، ولا يمكن تعديله بعدها.',
            'add_debit_line' => 'إضافة سطر مدين',
            'add_credit_line' => 'إضافة سطر دائن',
            'fill_remaining' => 'املأ الفرق المتبقي',
        ],

        'placeholders' => [
            'total_debit' => 'إجمالي المدين',
            'total_credit' => 'إجمالي الدائن',
            'difference' => 'الفرق (يجب أن يكون صفراً)',
            'balanced' => 'القيد متوازن',
        ],

        'notifications' => [
            'posted' => 'تم ترحيل القيد.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | دفتر الأستاذ - General Ledger
    |--------------------------------------------------------------------------
    */
    'general_ledger' => [
        'title' => 'دفتر الأستاذ',
        'line_label' => 'حركة',
        'lines_label' => 'حركات الدفتر',
        'empty' => 'لا توجد حركات مُرحَّلة في الدفتر بعد.',
        'opening_balance' => 'رصيد أول المدة',
        'total' => 'الإجمالي',

        'columns' => [
            'date' => 'التاريخ',
            'document_number' => 'رقم المستند',
            'document_type' => 'المستند',
            'description' => 'البيان',
            'debit' => 'مدين',
            'credit' => 'دائن',
            'balance' => 'الرصيد',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | ميزان المراجعة - Trial Balance
    |--------------------------------------------------------------------------
    */
    /*
    |--------------------------------------------------------------------------
    | اليومية التحليلية — قائمة المواد سلايد 2
    |--------------------------------------------------------------------------
    */
    'journal_daybook' => [
        'label' => 'اليومية التحليلية',
        'navigation_label' => 'اليومية التحليلية',
        'title' => 'اليومية التحليلية',
        'period' => 'من :from إلى :to',
        'totals' => 'الإجماليات',
        'empty' => 'لا توجد قيود مُرحَّلة في هذه المدة.',

        'filters' => [
            'from' => 'من تاريخ',
            'to' => 'إلى تاريخ',
            'currency' => 'العملة',
            'accounts' => 'أعمدة الحسابات',
            'accounts_hint' => 'حتى :max حسابات — اتركها فارغة لأكثر الحسابات حركة',
            'clear_accounts' => 'إعادة التعيين',
        ],

        'columns' => [
            'date' => 'التاريخ',
            'entry_serial' => 'رقم القيد',
            'document_number' => 'رقم المستند',
            'description' => 'البيــــــان',
            'debit' => 'مدين',
            'credit' => 'دائن',
            'entry_totals' => 'إجمالي القيد',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | كشف حساب / دفتر الأستاذ — قائمة المواد سلايد 3
    |--------------------------------------------------------------------------
    */
    'general_ledger_report' => [
        'label' => 'كشف حساب',
        'navigation_label' => 'كشف حساب (دفتر الأستاذ)',
        'title' => 'كشف حساب',
        'heading' => 'حساب :account',
        'period' => 'من :from إلى :to',
        'opening_balance' => 'رصيد أول المدة',
        'closing_balance' => 'رصيد آخر المدة',
        'totals' => 'الإجمالي',
        'empty' => 'لا توجد حركات مُرحَّلة في هذه المدة.',
        'no_account' => 'اختر حساباً لعرض كشفه.',

        'filters' => [
            'account' => 'الحساب',
            'from' => 'من تاريخ',
            'to' => 'إلى تاريخ',
        ],

        'columns' => [
            'date' => 'التاريخ',
            'entry_serial' => 'رقم القيد',
            'document_number' => 'رقم المستند',
            'description' => 'البيــــــان',
            'debit' => 'مدين',
            'credit' => 'دائن',
            'balance' => 'الرصيد',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | مقارنة الخامات (أمر التشغيل مقابل أوامر الصرف) — قائمة المواد سلايد 1
    |--------------------------------------------------------------------------
    */
    'material_variance' => [
        'heading' => 'مقارنة الخامات — أمر التشغيل :order',
        'planned_value' => 'قيمة المخطط',
        'issued_value' => 'قيمة المنصرف',
        'loss_value' => 'قيمة الفرق (الهالك)',
        'empty' => 'لا توجد خامات مخططة أو منصرفة لهذا الأمر.',

        'columns' => [
            'item' => 'الخامة',
            'planned' => 'المخطط (قائمة المواد)',
            'issued' => 'المنصرف',
            'returned' => 'المرتد',
            'net_issued' => 'صافي المنصرف',
            'variance' => 'الفرق',
            'variance_percentage' => 'نسبة الفرق',
            'variance_value' => 'قيمة الفرق',
        ],
    ],

    'trial_balance' => [
        'label' => 'ميزان المراجعة',
        'navigation_label' => 'ميزان المراجعة',
        'title' => 'ميزان المراجعة',
        'as_of' => 'حتى تاريخ',
        'currency' => 'العملة',
        'totals' => 'الإجماليات',
        'balanced' => 'ميزان المراجعة متوازن.',
        'unbalanced' => 'تنبيه: ميزان المراجعة غير متوازن.',
        'empty' => 'لا توجد حسابات للعرض.',

        'columns' => [
            'row' => 'م',
            'account' => 'اسم الحساب',
            'debit' => 'مدين',
            'credit' => 'دائن',
            'balance' => 'الرصيد',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | القوائم المالية — ما بعد ميزان المراجعة (ماليات.pptx)
    |--------------------------------------------------------------------------
    */

    // نصوص مشتركة بين القوائم الأربع
    'financial_statements' => [
        'filters' => [
            'from' => 'من تاريخ',
            'to' => 'إلى تاريخ',
            'period_from' => 'بداية الفترة المالية',
            'as_of' => 'المركز في تاريخ',
        ],
        'columns' => [
            'line' => 'البيان',
            'description' => 'البيان',
            'account_code' => 'كود الحساب',
            'amount' => 'القيمة',
            'partial' => 'جزئي',
            'total' => 'كلي',
        ],
        'no_rows' => 'لا توجد حسابات في هذا القسم.',
        'no_movement' => 'لا توجد حركة في هذه الفترة.',
        'pdf' => [
            'period' => 'عن الفترة من :from إلى :to',
            'as_of' => 'في :date',
            'footer' => 'تقرير آلي من منصة إلكتروتك — القوائم المالية',
        ],
    ],

    'operating_statement' => [
        'label' => 'قائمة التشغيل',
        'plural_label' => 'قوائم التشغيل',
        'navigation_label' => 'قائمة التشغيل',
        'title' => 'قائمة التشغيل',
        'description' => 'مكوّنات تكلفة المبيعات: تكلفة البضاعة المباعة + مصروفات التشغيل + مصروفات التركيب + مصروفات التصدير + المصروفات والإهلاكات الصناعية.',
        'cost_of_sales' => 'تكلفة المبيعات',
        'chart_note' => 'كل سطر هنا حساب مسجَّل في شجرة الحسابات ومصنَّف على بند «تكلفة المبيعات». لإضافة سطر جديد أضف الحساب وصنّفه على نفس البند.',
        'empty' => 'لا توجد حسابات مصنَّفة على بند تكلفة المبيعات.',
    ],

    'income_statement' => [
        'label' => 'قائمة الدخل',
        'plural_label' => 'قوائم الدخل',
        'navigation_label' => 'قائمة الدخل',
        'title' => 'قائمة الدخل',
        'description' => 'من صافي المبيعات إلى صافي الربح، بعمودَي «جزئي» و«كلي».',
        'detail_heading' => 'تفاصيل الحسابات وراء كل سطر',
        'formula_note' => 'مجمل الربح = صافي المبيعات − تكلفة المبيعات · إجمالي الإيرادات = الإيرادات المتنوعة + الأرباح الرأسمالية + فروق العملة الدائنة · صافي الربح = مجمل الربح + إجمالي الإيرادات − إجمالي المصروفات. علامة ( − ) تبيّن طبيعة الحساب فقط؛ الأرقام موجبة وتُطرح مرة واحدة في العمود الكلي.',

        'rows' => [
            'net_sales' => 'صافي المبيعات (حساب المبيعات − المردودات)',
            'sales' => 'المبيعات',
            'sales_returns' => 'مردودات المبيعات',
            'cost_of_sales' => 'تكلفة المبيعات',
            'gross_profit' => 'مجمل الربح',
            'other_revenue' => 'إيرادات متنوعة',
            'capital_gains' => 'أرباح رأسمالية',
            'fx_gain' => 'فروق العملة (دائنة)',
            'total_revenues' => 'إجمالي الإيرادات',
            'general_admin' => 'مصروفات عمومية',
            'fx_loss' => 'فروق العملة (مدينة)',
            'depreciation' => 'مصروفات وإهلاكات',
            'closed_letters_of_credit' => 'اعتمادات مستندية سبق إقفالها',
            'finance_cost' => 'فوائد مدينة',
            'total_expenses' => 'إجمالي المصروفات',
            'net_profit' => 'صافي الربح',
        ],
    ],

    'balance_sheet' => [
        'label' => 'قائمة المركز المالي',
        'plural_label' => 'قوائم المركز المالي',
        'navigation_label' => 'قائمة المركز المالي',
        'title' => 'قائمة المركز المالي',
        'as_of_label' => 'المركز المالي في :date',
        'balanced' => 'القائمة متوازنة: إجمالي الاستثمار = إجمالي التمويل.',
        'unbalanced' => 'غير متوازنة — الفرق :difference',
        'unbalanced_hint' => 'الفرق بين إجمالي الاستثمار وإجمالي التمويل هو :difference. الأسباب المعتادة: حساب غير مصنَّف على بند صحيح، أو قيد غير مرحَّل، أو رصيد أول مدة ناقص. راجع شجرة الحسابات وفلتر «حسابات غير مصنَّفة».',
        'period_profit' => 'أرباح الفترة',
        'from_income_statement' => 'من قائمة الدخل',
        'unlinked_accumulated_depreciation' => 'مجمع إهلاك غير مرتبط بأصل',
        'memo_line' => 'يوجد :account بمبلغ :amount',

        'sections' => [
            'long_term_assets' => 'الأصول طويلة الأجل (الأصول الثابتة)',
            'current_assets' => 'الأصول المتداولة',
            'current_liabilities' => 'الالتزامات المتداولة',
            'equations' => 'المعادلات',
            'funding' => 'يتم تمويله على النحو التالي (حقوق الملكية)',
            'funding_hint' => 'رأس المال + الاحتياطيات والمخصصات + جاري شركاء + أرباح الفترة.',
            'memo' => 'خارج المعادلات (حسابات نظامية)',
            'memo_hint' => 'حسابات لا تدخل رأس المال العامل ولا إجمالي الاستثمار، وتُذكر كحاشية أسفل القائمة.',
        ],

        'columns' => [
            'cost' => 'التكلفة',
            'accumulated_depreciation' => 'مجمع الإهلاك',
            'net' => 'الصافي',
        ],

        'cards' => [
            'working_capital' => 'رأس المال العامل',
            'fixed_assets_net' => 'صافي الأصول الثابتة',
            'total_investment' => 'إجمالي الاستثمار',
            'total_funding' => 'إجمالي التمويل',
        ],

        'rows' => [
            'total_long_term_assets' => 'مجموع الأصول طويلة الأجل',
            'total_current_assets' => 'مجموع الأصول المتداولة',
            'total_current_liabilities' => 'مجموع الالتزامات المتداولة',
            'working_capital_formula' => 'رأس المال العامل = الأصول المتداولة − الالتزامات المتداولة',
            'total_investment_formula' => 'إجمالي الاستثمار = رأس المال العامل + الأصول الثابتة',
            'total_funding' => 'إجمالي حقوق الملكية والتمويل',
        ],

        // سلايد 7 — تقسيم العملاء والموردين
        'party' => [
            'customer' => [
                'debit' => 'عملاء (أرصدة مدينة)',
                'credit' => 'عملاء — دفعات مقدمة (أرصدة دائنة)',
            ],
            'supplier' => [
                'debit' => 'موردون — دفعات مقدمة (أرصدة مدينة)',
                'credit' => 'موردون (أرصدة دائنة)',
            ],
            'reconciliation' => 'فرق مطابقة :account (حساب المراقبة − دفتر الأطراف)',
            'variance_badge' => 'فرق مطابقة',
            'count' => ':count طرف',
            'detail_heading' => 'تفاصيل تقسيم العملاء والموردين',
            'detail_hint' => 'الطرف ذو الرصيد الموجب يظهر في الأصول، وذو الرصيد السالب في الالتزامات المتداولة.',
        ],
    ],

    'cash_flow_statement' => [
        'label' => 'قائمة التدفقات النقدية',
        'plural_label' => 'قوائم التدفقات النقدية',
        'navigation_label' => 'قائمة التدفقات النقدية',
        'title' => 'قائمة التدفقات النقدية',
        'description' => 'تبدأ بصافي الربح المأخوذ من قائمة الدخل، ثم تُعدَّل بالحسابات الأخرى للوصول إلى رصيد النقدية آخر الفترة.',
        'from_income_statement' => 'من قائمة الدخل',
        'reconciled' => 'مطابق مع الواقع',
        'not_reconciled' => 'غير مطابق — الفرق :difference',
        'case' => 'احتمال :case',
        'no_adjustment' => 'بدون تسوية',
        // User-facing copy only: no reference to the requirements document, and
        // no configuration key names — those belong in config/finance.php, not
        // on an accountant's screen.
        'formula_note' => [
            'heading_add_back' => 'الصيغة المستخدمة: المطابِقة (الافتراضية)',
            'body_add_back' => 'أرباح التشغيل = صافي الربح + الإهلاك والمخصصات، والأرباح الرأسمالية بدون تسوية لأن متحصلاتها دخلت النقدية بالفعل. هذه هي الصيغة الوحيدة التي يتطابق معها الرصيد المستخرج مع الرصيد الفعلي.',
            'heading_client' => 'الصيغة المستخدمة: البديلة (خصم الإهلاك والمخصصات)',
            'body_client' => 'أرباح التشغيل = صافي الربح − الإهلاك والمخصصات + الأرباح الرأسمالية. بهذه الصيغة لن يطابق الرصيد المستخرج الرصيدَ الفعلي، والفرق يساوي ضعف (الإهلاك + المخصصات) زائد الأرباح الرأسمالية.',
        ],

        'columns' => [
            'opening' => 'رصيد أول الفترة',
            'closing' => 'رصيد آخر الفترة',
            'effect' => 'الأثر النقدي',
        ],

        'sections' => [
            'operating' => 'التدفقات النقدية من أنشطة التشغيل',
            'investing' => 'التدفقات النقدية من أنشطة الاستثمار',
            'financing' => 'التدفقات النقدية من أنشطة التمويل',
        ],

        'rows' => [
            'net_profit' => 'صافي ربح الفترة',
            'depreciation' => 'الإهلاك',
            'provisions' => 'المخصصات',
            'capital_gains' => 'أرباح رأسمالية',
            'operating_profit_before_wc' => 'أرباح التشغيل قبل التغير في رأس المال العامل',
            'change_in' => 'النقص (الزيادة) في :account',
            'operating_cash' => 'صافي التدفقات النقدية من أنشطة التشغيل',
            'investing_total' => 'صافي التدفقات النقدية من أنشطة الاستثمار',
            'financing_total' => 'صافي التدفقات النقدية من أنشطة التمويل',
            'net_change' => 'صافي التغير في النقدية',
            'opening_cash' => 'النقدية وما في حكمها أول الفترة',
            'closing_cash' => 'النقدية وما في حكمها آخر الفترة',
        ],

        'reconciliation' => [
            'heading' => 'مطابقة النقدية مع الواقع',
            'description' => 'الرصيد المستخرج من القائمة يجب أن يساوي رصيد حسابات النقدية والبنوك فعلياً في نفس التاريخ.',
            'derived' => 'الرصيد المستخرج من القائمة',
            'actual' => 'الرصيد الفعلي (النقدية والبنوك)',
            'difference' => 'الفرق',
        ],
    ],

    // سلايد 7 — الخط الزمني للعملية
    'operation_timeline' => [
        'heading' => 'الخط الزمني للعملية',
        'description' => 'مراحل العملية من استلامها حتى إتمامها، مشتقة تلقائياً من مستندات النظام.',
        'not_reached' => 'لم تُنجَز بعد',
        'days_from_start' => 'بعد :days يوم من البداية',

        'stages' => [
            'not_started' => 'لم تبدأ بعد',
            'sales' => 'استلام العملية من المبيعات',
            'activated' => 'تنشيط العملية',
            'technical_office' => 'في مكتب إدارة المشروعات — إنشاء أمر التشغيل',
            'order_approved' => 'اعتماد أمر التشغيل',
            'in_factory' => 'في المصنع — بدء صرف الخامات',
            'manufacturing_finished' => 'انتهاء التصنيع',
            'qa_approved' => 'اعتماد الجودة',
            'delivered' => 'التسليم للعميل',
            'installed' => 'تم التركيب',
            'completed' => 'إتمام العملية',
        ],

        'details' => [
            'work_orders' => ':count أمر تشغيل',
        ],
    ],

    // سلايد 7 — طبيعة رصيد الطرف
    'parties' => [
        'nature' => [
            'debit' => 'مدين',
            'credit' => 'دائن',
            'settled' => 'مسدَّد',
        ],
        'nature_hint_customer' => 'الرصيد الموجب ⇐ عميل مدين (ضمن الأصول)، والسالب ⇐ عميل دائن / دفعة مقدمة (ضمن الالتزامات المتداولة).',
        'nature_hint_supplier' => 'الرصيد الموجب ⇐ مورد مدين / دفعة مقدمة (ضمن الأصول)، والسالب ⇐ مورد دائن (ضمن الالتزامات المتداولة).',
    ],

    /*
    |--------------------------------------------------------------------------
    | الإدارة العامة — نظرة عامة على العمليات (Operations Overview)
    |--------------------------------------------------------------------------
    */
    'operations_overview' => [
        'label' => 'نظرة عامة على العمليات',
        'plural_label' => 'نظرة عامة على العمليات',
        'navigation_label' => 'نظرة عامة على العمليات',
        'title' => 'العمليات النشطة',
        'empty' => 'لا توجد عمليات نشطة.',
        'filters' => [
            'search' => 'بحث بالاسم أو الكود أو العميل',
        ],
        'columns' => [
            'row' => 'م',
            'operation' => 'العملية',
            'client' => 'العميل',
            'estimated_budget' => 'الميزانية',
            'actual_cost' => 'التكلفة الفعلية',
            'usage' => 'استهلاك الميزانية',
            'boms' => 'قوائم المواد',
            'work_orders' => 'أوامر التصنيع',
            'purchase_orders' => 'أوامر الشراء',
            'deliveries' => 'التسليمات',
            'status' => 'الحالة',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | مركز تكلفة العملية (Operation Cost Center)
    |--------------------------------------------------------------------------
    */
    'operations_cost' => [
        'label' => 'ملف تكلفة العملية',
        'plural_label' => 'ملفات تكلفة العمليات',
        'navigation_label' => 'مركز تكلفة العملية',
        'title' => 'مركز تكلفة العملية',
        'select_operation' => 'اختر العملية',
        'empty' => 'اختر عملية لعرض مركز تكلفتها.',
        'cards' => [
            'estimated_budget' => 'الميزانية التقديرية',
            'materials_cost' => 'تكلفة الخامات',
            'ledger_expenses' => 'مصروفات الأستاذ',
            'installation_expenses' => 'مصاريف التركيب',
            'purchases_reference' => 'المشتريات (مرجعي)',
            'total_cost' => 'إجمالي التكلفة',
            'revenue' => 'الإيراد (التسليمات)',
            'received' => 'المحصَّل (نقدي)',
            'profit' => 'الربح',
            'margin' => 'هامش الربح',
            'inventory_consumed' => 'المستهلَك من المخزون',
            'closed_to_cogs' => 'المُقفَل في تكلفة البضاعة المباعة',
            'unclosed_cost' => 'الرصيد غير المقفَل',
        ],
        'closing' => [
            'heading' => 'إقفال مركز التكلفة',
            'description' => 'عند تسليم العميل يتم إقفال ما تحمله العملية من تكلفة داخل المخزون في حساب تكلفة البضاعة المباعة.',
            'status' => 'حالة الإقفال',
            'empty' => 'لم يُقفَل هذا المركز بعد.',
            'automatic' => 'النظام (عند التسليم)',
            'journal_description' => 'إقفال مركز تكلفة العملية :operation في تكلفة البضاعة المباعة',
            'reversal_journal_description' => 'عكس إقفال مركز تكلفة العملية :operation',
            'states' => [
                'open' => 'مفتوح',
                'partially_closed' => 'مقفل جزئياً',
                'closed' => 'مقفل',
            ],
            'columns' => [
                'date' => 'التاريخ',
                'amount' => 'القيمة',
                'entry' => 'رقم القيد',
                'delivery' => 'إذن التسليم',
                'by' => 'أقفله',
                'notes' => 'ملاحظات',
            ],
            'fields' => [
                'closing' => 'الإقفال المراد عكسه',
                'reason' => 'سبب العكس',
            ],
            'actions' => [
                'close' => 'إقفال مركز التكلفة',
                'close_description' => 'يُنشئ قيداً: مدين تكلفة البضاعة المباعة / دائن المخزون، بقيمة الرصيد أدناه.',
                'reverse' => 'عكس إقفال',
            ],
            'notifications' => [
                'closed' => 'تم إقفال مركز التكلفة في تكلفة البضاعة المباعة.',
                'closed_body' => 'القيمة: :amount — القيد :entry.',
                'reversed' => 'تم عكس الإقفال بقيد معاكس.',
            ],
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | إجراءات وإشعارات دورة حياة العملية (الإدارة العامة)
    |--------------------------------------------------------------------------
    */
    'operations' => [
        'actions' => [
            'complete' => 'إنهاء العملية',
            'hold' => 'تعليق العملية',
            'resume' => 'استئناف العملية',
        ],
        'notifications' => [
            'completed' => 'تم إنهاء العملية.',
            'held' => 'تم تعليق العملية.',
            'resumed' => 'تم استئناف العملية.',
            'activated_title' => 'عملية نشطة جديدة',
            'activated_body' => 'العملية :operation أصبحت نشطة وتم ترحيلها إلى جميع الأقسام.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | حجز الكمية للعملية (Stock Reservations)
    |--------------------------------------------------------------------------
    */
    'stock_reservations' => [
        'label' => 'حجز مخزون',
        'plural_label' => 'حجوزات المخزون',
        'navigation_label' => 'حجوزات المخزون',
        'columns' => [
            'operation' => 'العملية',
            'item' => 'الصنف',
            'warehouse' => 'المخزن',
            'quantity' => 'الكمية',
            'status' => 'الحالة',
            'released_at' => 'تاريخ التحرير',
        ],
        'fields' => [
            'operation' => 'العملية',
            'item' => 'الصنف',
            'warehouse' => 'المخزن',
            'quantity' => 'الكمية',
            'quantity_available' => 'المتاح للحجز: :qty',
            'notes' => 'ملاحظات',
        ],
        'actions' => [
            'reserve' => 'حجز مخزون',
            'release' => 'تحرير',
        ],
        'notifications' => [
            'reserved' => 'تم حجز المخزون للعملية.',
            'released' => 'تم تحرير الحجز.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | محاضر التسليم (Delivery Minutes)
    |--------------------------------------------------------------------------
    */
    'delivery_minutes' => [
        'label' => 'محضر تسليم',
        'plural_label' => 'محاضر التسليم',
        'navigation_label' => 'محاضر التسليم',
        'sections' => [
            'details' => 'بيانات المحضر',
        ],
        'fields' => [
            'minute_number' => 'رقم المحضر',
            'minute_date' => 'التاريخ',
            'operation' => 'العملية',
            'delivery_voucher' => 'إذن التسليم',
            'customer' => 'العميل',
            'content' => 'المحتوى',
        ],
        'columns' => [
            'minute_number' => 'رقم المحضر',
            'operation' => 'العملية',
            'customer' => 'العميل',
            'minute_date' => 'التاريخ',
            'distributed' => 'تم التوزيع',
        ],
        'actions' => [
            'distribute' => 'توزيع',
        ],
        'notifications' => [
            'distributed' => 'تم توزيع محضر التسليم على جميع الأقسام.',
            'distributed_title' => 'محضر تسليم',
            'distributed_body' => 'تم توزيع محضر التسليم :number للعملية :operation.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | المطالبة المالية (Financial Claims)
    |--------------------------------------------------------------------------
    */
    'financial_claims' => [
        'label' => 'مطالبة مالية',
        'plural_label' => 'المطالبات المالية',
        'navigation_label' => 'المطالبات المالية',
        'sections' => [
            'details' => 'بيانات المطالبة',
        ],
        'fields' => [
            'claim_number' => 'رقم المطالبة',
            'claim_date' => 'التاريخ',
            'operation' => 'العملية',
            'customer' => 'العميل',
            'amount' => 'المبلغ',
            'description' => 'الوصف',
        ],
        'columns' => [
            'claim_number' => 'رقم المطالبة',
            'operation' => 'العملية',
            'customer' => 'العميل',
            'amount' => 'المبلغ',
            'status' => 'الحالة',
            'claim_date' => 'التاريخ',
        ],
        'actions' => [
            'submit' => 'تقديم',
            'collect' => 'تحصيل',
        ],
        'notifications' => [
            'submitted' => 'تم تقديم المطالبة المالية.',
            'collected' => 'تم تحصيل المطالبة المالية.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | الدفعات والمقبوضات (Operation Payments)
    |--------------------------------------------------------------------------
    */
    'operation_payments' => [
        'label' => 'دفعة',
        'plural_label' => 'الدفعات والمقبوضات',
        'navigation_label' => 'الدفعات',
        'journal_description' => 'دفعة عملية :number',
        'sections' => [
            'details' => 'بيانات الدفعة',
        ],
        'fields' => [
            'payment_number' => 'رقم الدفعة',
            'payment_date' => 'التاريخ',
            'operation' => 'العملية',
            'customer' => 'العميل',
            'direction' => 'الاتجاه',
            'method' => 'الطريقة',
            'amount' => 'المبلغ',
            'currency' => 'العملة',
            'account' => 'حساب الخزينة / البنك',
            'account_hint' => 'مطلوب لتوليد قيد يومية تلقائي.',
            'counter_account' => 'الحساب المقابل',
            'financial_claim' => 'المطالبة المالية',
            'reference' => 'المرجع',
            'notes' => 'ملاحظات',
        ],
        'columns' => [
            'payment_number' => 'رقم الدفعة',
            'operation' => 'العملية',
            'customer' => 'العميل',
            'direction' => 'الاتجاه',
            'method' => 'الطريقة',
            'amount' => 'المبلغ',
            'payment_date' => 'التاريخ',
            'posted' => 'مُرحَّلة للأستاذ',
        ],
        'filters' => [
            'from' => 'من',
            'until' => 'إلى',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | ملف أوامر التوريد (Supply Orders File)
    |--------------------------------------------------------------------------
    */
    'supply_orders_file' => [
        'label' => 'ملف أوامر التوريد',
        'navigation_label' => 'ملف أوامر التوريد',
        'title' => 'ملف أوامر التوريد',
        'select_operation' => 'اختر العملية',
        'empty' => 'اختر عملية لعرض ملف أوامر التوريد الخاص بها.',
        'orders_heading' => 'أوامر الشراء / التوريد',
        'no_orders' => 'لا توجد أوامر شراء لهذه العملية.',
        'summary' => [
            'revenue' => 'الإيراد (التسليمات)',
            'received' => 'المحصَّل',
            'paid' => 'المدفوع',
            'outstanding' => 'المتبقّي',
        ],
        'columns' => [
            'po_number' => 'رقم الأمر',
            'supplier' => 'المورد',
            'status' => 'الحالة',
            'total' => 'الإجمالي',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | التسهيلات الائتمانية (Credit Facilities)
    |--------------------------------------------------------------------------
    */
    'credit_facilities' => [
        'label' => 'تسهيل ائتماني',
        'plural_label' => 'التسهيلات الائتمانية',
        'navigation_label' => 'التسهيلات الائتمانية',
        'sections' => [
            'details' => 'بيانات التسهيل',
        ],
        'fields' => [
            'name' => 'الاسم',
            'status' => 'الحالة',
            'account' => 'حساب الأستاذ',
            'customer' => 'العميل',
            'limit_amount' => 'السقف',
            'currency' => 'العملة',
            'start_date' => 'تاريخ البداية',
            'end_date' => 'تاريخ النهاية',
            'notes' => 'ملاحظات',
        ],
        'columns' => [
            'name' => 'الاسم',
            'account' => 'الحساب',
            'limit_amount' => 'السقف',
            'used_amount' => 'المستخدَم',
            'available_amount' => 'المتاح',
            'status' => 'الحالة',
        ],
    ],

    'facility_allocations' => [
        'label' => 'تخصيص',
        'plural_label' => 'تخصيصات العمليات',
        'fields' => [
            'operation' => 'العملية',
            'amount' => 'المبلغ المخصَّص',
            'notes' => 'ملاحظات',
        ],
        'columns' => [
            'operation' => 'العملية',
            'amount' => 'المبلغ',
            'allocated_at' => 'تاريخ التخصيص',
            'status' => 'الحالة',
        ],
        'actions' => [
            'allocate' => 'تخصيص لعملية',
            'release' => 'تحرير',
        ],
        'notifications' => [
            'allocated' => 'تم تخصيص التسهيل للعملية.',
            'released' => 'تم تحرير التخصيص.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | مرحلة التركيب (Installations)
    |--------------------------------------------------------------------------
    */
    'installations' => [
        'label' => 'تركيب',
        'plural_label' => 'التركيبات',
        'navigation_label' => 'التركيبات',
        'sections' => [
            'details' => 'بيانات التركيب',
        ],
        'fields' => [
            'operation' => 'العملية',
            'delivery_voucher' => 'إذن التسليم',
            'notes' => 'ملاحظات',
        ],
        'columns' => [
            'operation' => 'العملية',
            'status' => 'الحالة',
            'started_at' => 'تاريخ البدء',
            'completed_at' => 'تاريخ الإتمام',
        ],
        'actions' => [
            'start' => 'بدء التركيب',
            'complete' => 'إنهاء التركيب',
        ],
        'notifications' => [
            'started' => 'تم بدء التركيب.',
            'completed' => 'تم إنهاء التركيب.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | معاينات الموقع (Site Surveys)
    |--------------------------------------------------------------------------
    */
    'site_surveys' => [
        'label' => 'معاينة موقع',
        'plural_label' => 'معاينات الموقع',
        'navigation_label' => 'معاينات الموقع',
        'sections' => [
            'details' => 'بيانات المعاينة',
        ],
        'fields' => [
            'operation' => 'العملية',
            'survey_date' => 'تاريخ المعاينة',
            'surveyed_by' => 'القائم بالمعاينة',
            'measurements' => 'المقاسات',
            'notes' => 'ملاحظات',
        ],
        'columns' => [
            'operation' => 'العملية',
            'survey_date' => 'تاريخ المعاينة',
            'surveyed_by' => 'القائم بالمعاينة',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | إذن التسليم - Delivery Voucher Resource
    |--------------------------------------------------------------------------
    */
    'delivery_vouchers' => [
        'label' => 'إذن تسليم',
        'plural_label' => 'أذون التسليم',
        'navigation_label' => 'أذون التسليم',

        'sections' => [
            'details' => 'بيانات الإذن',
            'specs' => 'المواصفات الفنية',
            'lines' => 'الأصناف المُسلَّمة',
        ],

        'fields' => [
            'voucher_number' => 'رقم الإذن',
            'customer' => 'العميل',
            'project' => 'العملية / المشروع',
            'supply_order_number' => 'رقم طلب التوريد',
            'voucher_date' => 'التاريخ',
            'plates_count' => 'عدد الأطباق',
            'protection_degree' => 'درجة الحماية',
            'insulation_voltage' => 'جهد العزل',
            'notes' => 'ملاحظات',
            'lines' => 'الأصناف',
            'item' => 'الصنف',
            'quantity' => 'الكمية',
            'unit_cost' => 'تكلفة الوحدة',
            'non_invoice_reason' => 'سبب عدم الفوترة',
            'non_invoice_reason_hint' => 'مثل: عينات أو سحب شخصي.',
        ],

        'columns' => [
            'voucher_number' => 'الرقم',
            'customer' => 'العميل',
            'voucher_date' => 'التاريخ',
            'technical' => 'فني',
            'financial' => 'مالي',
            'total_value' => 'إجمالي القيمة',
            'invoiced_value' => 'القيمة المفوترة',
            'invoicing_status' => 'حالة الفوترة',
            'total_delivered' => 'إجمالي المُسلَّم',
            'total_invoiced' => 'إجمالي المفوتر',
            'status' => 'الحالة',
        ],

        'actions' => [
            'approve_technical' => 'اعتماد فني',
            'approve_financial' => 'اعتماد مالي',
            'record_invoice' => 'تسجيل فاتورة مبيعات',
            'set_non_invoice_reason' => 'سبب عدم الفوترة',
            'cancel' => 'إلغاء',
        ],

        'notifications' => [
            'approved' => 'تم تسجيل التوقيع. يُفعَّل الإذن عند اكتمال الاعتمادين.',
            'activated' => 'تم اكتمال الاعتمادين وتفعيل الإذن وخصم الأصناف من المخزون.',
            'invoice_recorded' => 'تم تسجيل الفاتورة وتحديث حالة فوترة الإذن.',
            'reason_saved' => 'تم حفظ السبب.',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | فواتير المبيعات - Sales Invoice Resource (سلايد 10)
    |--------------------------------------------------------------------------
    */
    'sales_invoices' => [
        'label' => 'فاتورة مبيعات',
        'plural_label' => 'فواتير المبيعات',
        'navigation_label' => 'فواتير المبيعات',

        'sections' => [
            'details' => 'بيانات الفاتورة',
        ],

        'fields' => [
            'delivery_voucher' => 'إذن التسليم',
            'invoice_number' => 'رقم الفاتورة',
            'invoice_date' => 'تاريخ الفاتورة',
            'amount' => 'القيمة',
            'remaining' => 'المتبقي للفوترة',
            'notes' => 'ملاحظات',
        ],

        'columns' => [
            'invoice_number' => 'رقم الفاتورة',
            'invoice_date' => 'التاريخ',
            'delivery_voucher' => 'إذن التسليم',
            'customer' => 'العميل',
            'voucher_invoicing_status' => 'حالة الإذن',
            'amount' => 'القيمة',
            'total_invoiced' => 'الإجمالي',
        ],

        'filters' => [
            'customer' => 'العميل',
            'from' => 'من تاريخ',
            'until' => 'إلى تاريخ',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | الإنتاج والفاقد - Production Entry Resource
    |--------------------------------------------------------------------------
    */
    'production_entries' => [
        'label' => 'إجراء إنتاج',
        'plural_label' => 'الإنتاج والفاقد',
        'navigation_label' => 'الإنتاج والفاقد',

        'columns' => [
            'work_order' => 'أمر التصنيع',
            'operation_name' => 'اسم العملية',
            'output_item' => 'المنتج',
            'entry_date' => 'التاريخ',
            // المقارنة القيمية (سلايد 9).
            'planned_cost' => 'المخطط',
            'actual_cost' => 'المنتج (الفعلي)',
            'loss_value' => 'الفاقد',
            'loss_percentage' => 'نسبة الفاقد',
            // أعمدة الكمية (اختيارية).
            'planned_quantity' => 'المخطط (كمية)',
            'produced_quantity' => 'المنتَج (كمية)',
            'scrap_quantity' => 'الفاقد (كمية)',
            'scrap_percentage' => 'نسبة الفاقد (كمية)',
        ],

        'filters' => [
            'from' => 'من',
            'until' => 'إلى',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | أوامر التصنيع - Work Order Resource
    |--------------------------------------------------------------------------
    */
    'work_orders' => [
        'label' => 'أمر تشغيل',
        'plural_label' => 'أوامر التصنيع',
        'navigation_label' => 'أوامر التصنيع',

        'sections' => [
            'wo_details' => 'تفاصيل أمر التصنيع',
            'outputs' => 'المنتجات التامة',
            'outputs_description' => 'أمر التصنيع الواحد ممكن ينتج أكتر من منتج، ولكل منتج كميته المخططة. مجموع الكميات دي هو الكمية المخططة للأمر، وبيها تتحسب الخامات المطلوبة وأذون الصرف.',
            'quantities_schedule' => 'الكميات والجدول الزمني',
            'materials' => 'جدول الخامات',
            'materials_description' => 'تُملأ تلقائياً من التركيبات القياسية لكل المنتجات التامة (كل منتج مُقاس على كميته، والخامات المشتركة تُجمَع في سطر واحد). عدّل الكميات بحرية لهذا الأمر — دون أن تتغيّر التركيبة القياسية.',
            'specs' => 'المواصفات الفنية',
            'specs_description' => 'مواصفات العملية المُدخَلة هنا تُنسَخ إلى بيانات العملية في ورقة الجودة عند إنشائها.',
            'qa_gate' => 'بوابة ضمان الجودة',
            'qa_gate_description' => 'اعتماد ضمان الجودة إلزامي قبل إتمام العملية.',
            'description' => 'الوصف',
        ],

        'fields' => [
            'wo_number' => 'رقم أمر التصنيع',
            'title' => 'العنوان',
            'project' => 'المشروع',
            'linked_bom' => 'قائمة المواد المرتبطة',
            'outputs' => 'المنتجات التامة',
            'output_item' => 'المنتج التام',
            'output_planned_quantity' => 'الكمية المخططة للمنتج',
            'output_item_helper' => 'الصنف الذي يُنتَج في مخزن التام عند إتمام أمر التصنيع.',
            'conductor_type' => 'نوع الموصل',
            'cross_section' => 'مساحة المقطع',
            'cross_section_e' => 'مساحة المقطع E',
            'external_body' => 'الجسم الخارجى',
            'protection_degree' => 'درجة الحماية',
            'paint' => 'الدهان',
            'model' => 'الطراز',
            'ampere' => 'الأمبير',
            'poles_count' => 'عدد الأقطاب',
            'status' => 'الحالة',
            'priority' => 'الأولوية',
            'assigned_to' => 'مسند إلى',
            'planned_quantity' => 'الكمية المخططة',
            'planned_quantity_helper' => 'محسوبة تلقائياً = مجموع كميات المنتجات التامة.',
            'produced_quantity' => 'الكمية المنتجة',
            'waste_quantity' => 'كمية الهالك',
            'finish_gate' => 'شروط انتهاء التصنيع',
            'materials' => 'الخامات',
            'material_item' => 'الخامة',
            'material_quantity' => 'الكمية',
            'material_unit_cost' => 'تكلفة الوحدة',
            'planned_material_cost' => 'تكلفة الخامات المخططة',
            'planned_start_date' => 'تاريخ البداية المخطط',
            'planned_end_date' => 'تاريخ النهاية المخطط',
            'qa_status' => 'حالة ضمان الجودة',
            'qa_notes' => 'ملاحظات ضمان الجودة',
            'order_approved_at' => 'اعتماد مكتب المشروعات',
            'manufacturing_finished_at' => 'انتهاء التصنيع',
            'manufacturing_duration' => 'مدة التصنيع',
            'manufacturing_finished_by' => 'أنهى التصنيع',
            'description' => 'الوصف',
        ],

        'columns' => [
            'wo_number' => 'رقم الأمر',
            'title' => 'العنوان',
            'project' => 'المشروع',
            'status' => 'الحالة',
            'priority' => 'الأولوية',
            'outputs' => 'المنتجات التامة',
            'planned' => 'المخطط',
            'produced' => 'المُنتَج',
            'estimated_cost' => 'التكلفة التقديرية',
            'actual_cost' => 'التكلفة الفعلية',
            'cost_variance' => 'انحراف التكلفة',
            'assigned_to' => 'مسند إلى',
            'start_date' => 'البداية',
            'finished_at' => 'انتهاء التصنيع',
            'duration' => 'مدة التصنيع',
        ],

        'actions' => [
            'approve_order' => 'اعتماد الأمر',
            'start' => 'بدء',
            'issue_materials' => 'صرف الخامات',
            'return_scrap' => 'ارتداد فضلات',
            'write_off_loss' => 'إهلاك',
            'submit_qa' => 'إرسال لضمان الجودة',
            'submit_qa_description' => 'سجّل الكمية المنتجة وكمية الهالك لكل منتج تام. دي المرحلة الوحيدة اللي بتتحدد فيها الكميات دي.',
            'approve_qa' => 'اعتماد الجودة',
            'complete' => 'إتمام',
            'finish_manufacturing' => 'انتهاء التصنيع',
            'finish_manufacturing_confirm' => 'إعلان انتهاء التصنيع بيبلّغ كل الأقسام إن المنتج جاهز للتسليم، وبيفتح ورقة الجودة. لازم يكون الأمر معتمَد من مكتب المشروعات ومن ضمان الجودة قبله.',
            'quality_sheet' => 'ورقة الجودة',
            'add_output' => 'إضافة منتج تام',
            'fetch_standard_materials' => 'جلب الخامات القياسية',
            'fetch_standard_materials_confirm' => 'سيتم استبدال جدول الخامات بالكامل بالتركيبات القياسية لكل المنتجات التامة — بما في ذلك أي تعديلات يدوية.',
            'material_variance' => 'مقارنة الخامات',
        ],

        'notifications' => [
            'order_approved' => 'تم اعتماد الأمر — أصبح جاهزاً للتصنيع',
            'started' => 'تم بدء أمر التصنيع',
            'materials_fetched' => 'تم جلب الخامات القياسية من قائمة المواد',
            'materials_not_refreshed' => 'جدول الخامات لم يُحدَّث',
            'materials_not_refreshed_body' => 'فيه كميات معدّلة يدوياً، فما اتدهستش. لو عايز تعيد الحساب من التركيبات القياسية استخدم زر «جلب الخامات القياسية».',
            'no_standard_bom_for' => 'منتجات بدون تركيبة قياسية معتمدة',
            'issue_voucher_created' => 'تم إنشاء إذن صرف (مسودة)',
            'return_voucher_created' => 'تم إنشاء إذن ارتداد (مسودة)',
            'depreciation_voucher_created' => 'تم إنشاء إذن إهلاك (مسودة)',
            'submitted_qa' => 'تم الإرسال لضمان الجودة',
            'qa_approved' => 'تم اعتماد الجودة',
            'completed' => 'تم إتمام أمر التصنيع',
            'manufacturing_finished' => 'تم إنهاء التصنيع — وتم تنبيه الأقسام',
            'ready_for_delivery_title' => 'المنتج جاهز للتسليم',
            'ready_for_delivery_body' => 'انتهى تصنيع :product (أمر التصنيع :wo_number) وأصبح جاهزاً للتسليم.',
            'view_work_order' => 'عرض أمر التصنيع',
            'failed' => 'فشل',
        ],

        'qa' => [
            'approved_by' => '✅ اعتُمد بواسطة :name في :date',
            'pending' => '⏳ في انتظار مراجعة الجودة',
        ],

        'order_approval' => [
            'approved_by' => '✅ اعتُمد بواسطة :name في :date',
            'pending' => '⏳ مسودة — في انتظار اعتماد مدير مكتب المشروعات',
        ],

        'manufacturing' => [
            'finished_at' => '🏁 انتهى في :date · المدة: :duration · بواسطة :name',
            'not_finished' => '⏳ التصنيع جارٍ',
            'finish_allowed' => '✅ الاعتمادان تمّا — زر «انتهاء التصنيع» متاح.',
            'finish_blocked' => '🔒 «انتهاء التصنيع» مقفول: لازم اعتماد مكتب المشروعات واعتماد ضمان الجودة الاتنين قبله.',
        ],

        'priority_options' => [
            'low' => 'منخفضة',
            'normal' => 'عادية',
            'high' => 'عالية',
            'urgent' => 'عاجلة',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | ورقة الجودة - Quality Sheet Resource
    |--------------------------------------------------------------------------
    */
    'quality_sheets' => [
        'label' => 'ورقة جودة',
        'plural_label' => 'أوراق الجودة',
        'navigation_label' => 'أوراق الجودة',

        'sections' => [
            'details' => 'بيانات العملية',
            'lines' => 'اختبارات الجودة',
            'lines_description' => 'صف لكل قطعة مفحوصة — تُسجَّل الجودة الظاهرية والمقاس المطلوب ونتائج اختبارات العزل/الاستمرارية.',
            'approval' => 'الاعتماد',
        ],

        'line_label' => 'بند رقم (:no)',

        'fields' => [
            'sheet_number' => 'رقم الورقة',
            'work_order' => 'أمر التصنيع',
            'test_date' => 'تاريخ الاختبار',
            'operation_name' => 'اسم العملية',
            'conductor_type' => 'نوع الموصل',
            'cross_section' => 'مساحة المقطع',
            'cross_section_e' => 'مساحة المقطع E',
            'external_body' => 'الجسم الخارجى',
            'protection_degree' => 'درجة الحماية',
            'paint' => 'الدهان',
            'model' => 'الطراز',
            'ampere' => 'الأمبير',
            'poles_count' => 'عدد الأقطاب',
            'notes' => 'ملاحظات',
            'lines' => 'بنود الاختبار',
            'piece_number' => 'رقم القطعة',
            'assembly' => 'التجميع',
            'visual_quality' => 'الجودة الظاهرية',
            'earth_bond_pe_fe' => 'تربيط الجانب الأرضى (PE–FE)',
            'required_size' => 'المقاس المطلوب',
            'reading_1' => 'قراءة 1',
            'reading_2' => 'قراءة 2',
            'test_pe_l123n' => 'PE-(L1&L2&L3&N)',
            'test_fe_l123n' => 'FE-(L1&L2&L3&N)',
            'test_n_l12l3' => 'N-(L1&L2&L3)',
            'test_l1_l2l3' => 'L1-(L2&L3)',
            'test_l2_l3' => 'L2-L3',
            'line_notes' => 'ملاحظات',
            'qa_fill_status' => 'حالة ملء الجودة',
            'factory_approval_status' => 'اعتماد مدير المصنع',
            'qa_inspector_notes' => 'ملاحظات مراقب الجودة',
        ],

        'columns' => [
            'sheet_number' => 'الرقم',
            'work_order' => 'أمر التصنيع',
            'status' => 'الحالة',
            'qa_filled_by' => 'مَلأها',
            'factory_approved_by' => 'اعتمدها',
            'test_date' => 'تاريخ الاختبار',
        ],

        'actions' => [
            'fill' => 'ملء (الجودة)',
            'approve' => 'اعتماد (مدير المصنع)',
            'approve_confirm' => 'الاعتماد النهائي يوقّع ورقة الجودة ويُنبّه كل الأقسام أن العملية تم الانتهاء من تصنيعها. لا يمكن التراجع.',
            'print' => 'طباعة',
            'print_ar' => 'طباعة (عربي)',
            'print_en' => 'طباعة (إنجليزي)',
        ],

        'notifications' => [
            'filled' => 'تم ملء ورقة الجودة وتوقيعها من قسم الجودة.',
            'approved' => 'تم اعتماد ورقة الجودة — تم تنبيه الأقسام.',
            'approved_alert_title' => 'تم الانتهاء من تصنيع العملية',
            'approved_alert_body' => 'تم اعتماد ورقة الجودة :sheet_number (أمر التصنيع :wo_number) — تم الانتهاء من تصنيع العملية.',
            'view_quality_sheet' => 'عرض ورقة الجودة',
        ],

        'status_lines' => [
            'qa_filled' => '✅ مَلأها :name بتاريخ :date',
            'qa_pending' => '⏳ في انتظار قسم الجودة',
            'approved' => '✅ اعتمدها :name بتاريخ :date',
            'approval_pending' => '⏳ في انتظار اعتماد مدير المصنع',
        ],

        'pdf' => [
            'company_name' => 'الإلكتروتك لصناعات المنتجات الكهربائية',
            'title' => 'ورقة اختبار التصنيع',
            'sheet_number' => 'رقم الورقة',
            'test_date' => 'التاريخ',
            'work_order' => 'أمر التصنيع',
            'operation_name' => 'العملية',
            'conductor_type' => 'نوع الموصل',
            'poles_count' => 'الأقطاب',
            'cross_section' => 'مساحة المقطع',
            'cross_section_e' => 'مساحة المقطع E',
            'external_body' => 'الجسم الخارجى',
            'protection_degree' => 'درجة الحماية',
            'paint' => 'الدهان',
            'model' => 'الطراز',
            'ampere' => 'الأمبير',
            'status' => 'الحالة',
            'line_no' => 'م',
            'piece_number' => 'رقم القطعة',
            'assembly' => 'التجميع',
            'visual_quality' => 'الجودة الظاهرية',
            'earth_bond_pe_fe' => 'تربيط الأرضى (PE–FE)',
            'required_size' => 'المقاس المطلوب',
            'test_pe_l123n' => 'PE-(L1&L2&L3&N)',
            'test_fe_l123n' => 'FE-(L1&L2&L3&N)',
            'test_n_l12l3' => 'N-(L1&L2&L3)',
            'test_l1_l2l3' => 'L1-(L2&L3)',
            'test_l2_l3' => 'L2-L3',
            'notes' => 'ملاحظات',
            'inspector_notes' => 'ملاحظات مراقب الجودة',
            'qa_inspector' => 'إمضاء مراقب الجودة',
            'factory_manager' => 'إمضاء مدير المصنع',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | المستخدمون - User Resource
    |--------------------------------------------------------------------------
    */
    'users' => [
        'label' => 'مستخدم',
        'plural_label' => 'المستخدمون',
        'navigation_label' => 'المستخدمون',

        'sections' => [
            'account_information' => 'معلومات الحساب',
        ],

        'fields' => [
            'name' => 'الاسم',
            'email' => 'البريد الإلكتروني',
            'password' => 'كلمة المرور',
            'password_helper_edit' => 'اتركه فارغاً للإبقاء على كلمة المرور الحالية.',
            'roles' => 'الدور',
            'roles_helper' => 'يحدد ما يمكن للمستخدم الوصول إليه في النظام.',
        ],

        'columns' => [
            'name' => 'الاسم',
            'email' => 'البريد الإلكتروني',
            'role' => 'الدور',
            'verified' => 'التحقق',
            'joined' => 'تاريخ الانضمام',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | الأدوار والصلاحيات - Role Resource
    |--------------------------------------------------------------------------
    */
    'roles' => [
        'label' => 'دور',
        'plural_label' => 'الأدوار',
        'navigation_label' => 'الأدوار والصلاحيات',

        'sections' => [
            'role_details' => 'تفاصيل الدور',
            'permissions' => 'الصلاحيات',
        ],

        'fields' => [
            'name' => 'اسم الدور',
            'permissions' => 'الصلاحيات الممنوحة',
            'permissions_helper' => 'حدد الصلاحيات التي يجب أن يحصل عليها هذا الدور.',
        ],

        'columns' => [
            'name' => 'الاسم',
            'permissions_count' => 'عدد الصلاحيات',
            'created_at' => 'تاريخ الإنشاء',
        ],

        'notifications' => [
            'admin_protected' => 'لا يمكن تعديل أو حذف دور المشرف العام.',
        ],

        'groups' => [
            'projects' => 'إدارة المشاريع',
            'project_offers' => 'عروض المشاريع',
            'items' => 'الأصناف',
            'boms' => 'قوائم المواد',
            'inventory' => 'مراقبة المخزون',
            'transactions' => 'حركات المخزون',
            'addition_vouchers' => 'أذون الإضافة',
            'issue_vouchers' => 'أذون الصرف',
            'return_vouchers' => 'أذون الارتداد',
            'depreciation_vouchers' => 'أذون الإهلاك',
            'delivery_vouchers' => 'أذون التسليم',
            'production_entries' => 'الإنتاج والفاقد',
            'scrap' => 'الفاقد',
            'purchase_orders' => 'أوامر الشراء',
            'suppliers' => 'الموردون',
            'customers' => 'العملاء',
            'supplier_statements' => 'كشوف حساب الموردين',
            'customer_statements' => 'كشوف حساب العملاء',
            'sales_invoices' => 'فواتير المبيعات',
            'accounts' => 'شجرة الحسابات',
            'journal_entries' => 'قيود اليومية',
            'journal_daybook' => 'اليومية التحليلية',
            'general_ledger' => 'دفتر الأستاذ',
            'trial_balance' => 'ميزان المراجعة',
            'operating_statement' => 'قائمة التشغيل',
            'income_statement' => 'قائمة الدخل',
            'balance_sheet' => 'قائمة المركز المالي',
            'cash_flow_statement' => 'قائمة التدفقات النقدية',
            'operations' => 'العمليات (الإدارة العامة)',
            'delivery_minutes' => 'محاضر التسليم',
            'financial_claims' => 'المطالبات المالية',
            'operation_payments' => 'الدفعات والمقبوضات',
            'supply_orders_file' => 'ملف أوامر التوريد',
            'credit_facilities' => 'التسهيلات الائتمانية',
            'installations' => 'التركيبات',
            'site_surveys' => 'معاينات الموقع',
            'work_orders' => 'أوامر التصنيع',
            'quality_sheets' => 'أوراق الجودة',
            'users' => 'إدارة المستخدمين',
            'roles' => 'إدارة الأدوار',
            'activity_log' => 'سجلات النشاط',
            'dashboard' => 'لوحة التحكم',
            'attachments' => 'المرفقات',
        ],

        'permissions' => [
            'projects' => [
                'view' => 'عرض المشاريع',
                'create' => 'إنشاء مشروع',
                'edit' => 'تعديل المشاريع',
                'delete' => 'حذف المشاريع',
                'change_status' => 'تغيير حالة المشروع',
                'move_to_tender' => 'نقل المشروع إلى المناقصات',
                'move_to_inhand' => 'نقل المشروع إلى تحت اليد',
                'move_to_active' => 'نقل المشروع إلى نشط',
                'cancel_to_lost' => 'نقل المشروع إلى المفقودة',
                'set_alarm' => 'ضبط تنبيه المشروع',
                'manager_approve' => 'موافقة المدير على المشروع',
                'view_history' => 'عرض سجل العملية',
            ],
            'project_offers' => [
                'view' => 'عرض عروض المشاريع',
                'create' => 'إنشاء عرض مشروع',
                'edit' => 'تعديل عروض المشاريع',
                'delete' => 'حذف عروض المشاريع',
                'print' => 'طباعة العرض',
            ],
            'attachments' => [
                'upload' => 'رفع المرفقات',
                'download' => 'تحميل المرفقات',
                'delete' => 'حذف المرفقات',
            ],
            'items' => [
                'view' => 'عرض الأصناف',
                'create' => 'إنشاء صنف',
                'edit' => 'تعديل الأصناف',
                'delete' => 'حذف الأصناف',
            ],
            'boms' => [
                'view' => 'عرض قوائم المواد',
                'create' => 'إنشاء قائمة مواد',
                'edit' => 'تعديل قوائم المواد',
                'approve' => 'اعتماد قوائم المواد',
                'delete' => 'حذف قوائم المواد',
            ],
            'inventory' => [
                'view' => 'عرض المخزون',
                'manage' => 'إدارة المخزون',
                'hold' => 'حجز المخزون',
                'release' => 'الإفراج عن المخزون',
                'transfer' => 'التحويل بين المخازن',
                'view_pricing' => 'عرض الأسعار',
            ],
            'transactions' => [
                'view' => 'عرض حركات المخزون',
            ],
            'addition_vouchers' => [
                'view' => 'عرض أذون الإضافة',
                'create' => 'إنشاء إذن إضافة',
                'post' => 'ترحيل إذن الإضافة',
                'invoice' => 'تسجيل/إقفال فاتورة المشتريات',
            ],
            'issue_vouchers' => [
                'view' => 'عرض أذون الصرف',
                'create' => 'إنشاء إذن صرف',
                'post' => 'ترحيل إذن الصرف',
                'approve_excess' => 'اعتماد صرف كمية زائدة',
            ],
            'return_vouchers' => [
                'view' => 'عرض أذون الارتداد',
                'create' => 'إنشاء إذن ارتداد',
                'post' => 'ترحيل إذن الارتداد',
            ],
            'depreciation_vouchers' => [
                'view' => 'عرض أذون الإهلاك',
                'create' => 'إنشاء إذن إهلاك',
                'post' => 'ترحيل إذن الإهلاك',
            ],
            'delivery_vouchers' => [
                'view' => 'عرض أذون التسليم',
                'create' => 'إنشاء إذن تسليم',
                'approve_technical' => 'الاعتماد الفني',
                'approve_financial' => 'الاعتماد المالي',
                'cancel' => 'إلغاء إذن التسليم',
            ],
            'sales_invoices' => [
                'view' => 'عرض فواتير المبيعات',
                'create' => 'تسجيل فاتورة مبيعات',
                'edit' => 'تعديل فواتير المبيعات',
                'delete' => 'حذف فواتير المبيعات',
            ],
            'production_entries' => [
                'view' => 'عرض الإنتاج والفاقد',
            ],
            'scrap' => [
                'view' => 'عرض الفاقد',
            ],
            'purchase_orders' => [
                'view' => 'عرض أوامر الشراء',
                'create' => 'إنشاء أمر شراء',
                'edit' => 'تعديل أوامر الشراء',
                'approve' => 'اعتماد أوامر الشراء',
                'receive' => 'استلام أوامر الشراء',
                'print' => 'طباعة أوامر الشراء',
                'delete' => 'حذف أوامر الشراء',
            ],
            'suppliers' => [
                'view' => 'عرض الموردين',
                'create' => 'إنشاء مورد',
                'edit' => 'تعديل الموردين',
                'delete' => 'حذف الموردين',
            ],
            'customers' => [
                'view' => 'عرض العملاء',
                'create' => 'إنشاء عميل',
                'edit' => 'تعديل العملاء',
                'delete' => 'حذف العملاء',
            ],
            'supplier_statements' => [
                'view' => 'عرض كشوف حساب الموردين',
            ],
            'customer_statements' => [
                'view' => 'عرض كشوف حساب العملاء',
            ],
            'accounts' => [
                'view' => 'عرض شجرة الحسابات',
                'create' => 'إنشاء حساب',
                'edit' => 'تعديل الحسابات',
                'delete' => 'حذف الحسابات',
            ],
            'journal_entries' => [
                'view' => 'عرض قيود اليومية',
                'create' => 'إنشاء قيد يومية',
                'edit' => 'تعديل قيود اليومية',
                'post' => 'ترحيل قيود اليومية',
                'delete' => 'حذف قيود اليومية',
            ],
            'journal_daybook' => [
                'view' => 'عرض اليومية التحليلية',
            ],
            'general_ledger' => [
                'view' => 'عرض دفتر الأستاذ',
            ],
            'trial_balance' => [
                'view' => 'عرض ميزان المراجعة',
            ],
            'operating_statement' => [
                'view' => 'عرض قائمة التشغيل',
            ],
            'income_statement' => [
                'view' => 'عرض قائمة الدخل',
            ],
            'balance_sheet' => [
                'view' => 'عرض قائمة المركز المالي',
            ],
            'cash_flow_statement' => [
                'view' => 'عرض قائمة التدفقات النقدية',
            ],
            'operations' => [
                'overview' => 'عرض نظرة عامة على العمليات',
                'view_cost' => 'عرض مركز تكلفة العملية',
                'close_cost_center' => 'إقفال مركز تكلفة العملية',
                'activate' => 'تنشيط العملية',
                'complete' => 'إنهاء العملية',
                'hold' => 'تعليق العملية',
                'reserve' => 'حجز مخزون للعملية',
            ],
            'delivery_minutes' => [
                'view' => 'عرض محاضر التسليم',
                'create' => 'إنشاء محضر تسليم',
                'distribute' => 'توزيع محضر التسليم',
            ],
            'financial_claims' => [
                'view' => 'عرض المطالبات المالية',
                'create' => 'إنشاء مطالبة مالية',
                'submit' => 'تقديم مطالبة مالية',
                'collect' => 'تحصيل مطالبة مالية',
            ],
            'operation_payments' => [
                'view' => 'عرض الدفعات والمقبوضات',
                'record' => 'تسجيل دفعة',
            ],
            'supply_orders_file' => [
                'view' => 'عرض ملف أوامر التوريد',
            ],
            'credit_facilities' => [
                'view' => 'عرض التسهيلات الائتمانية',
                'manage' => 'إدارة التسهيلات الائتمانية',
            ],
            'installations' => [
                'view' => 'عرض التركيبات',
                'manage' => 'إدارة التركيبات',
            ],
            'site_surveys' => [
                'view' => 'عرض معاينات الموقع',
                'manage' => 'إدارة معاينات الموقع',
            ],
            'work_orders' => [
                'view' => 'عرض أوامر التصنيع',
                'create' => 'إنشاء أمر تشغيل',
                'edit' => 'تعديل أوامر التصنيع',
                'approve_order' => 'اعتماد أمر التصنيع',
                'start' => 'بدء أمر التصنيع',
                'submit_qa' => 'إرسال لضمان الجودة',
                'approve_qa' => 'اعتماد ضمان الجودة',
                'complete' => 'إتمام أمر التصنيع',
                'finish_manufacturing' => 'إنهاء التصنيع',
                'delete' => 'حذف أوامر التصنيع',
            ],
            'quality_sheets' => [
                'view' => 'عرض أوراق الجودة',
                'create' => 'إنشاء ورقة جودة',
                'fill' => 'ملء ورقة الجودة (الجودة)',
                'approve' => 'اعتماد ورقة الجودة',
                'print' => 'طباعة ورقة الجودة',
            ],
            'users' => [
                'view' => 'عرض المستخدمين',
                'create' => 'إنشاء مستخدم',
                'edit' => 'تعديل المستخدمين',
                'delete' => 'حذف المستخدمين',
            ],
            'roles' => [
                'manage' => 'إدارة الأدوار والصلاحيات',
            ],
            'activity_log' => [
                'view' => 'عرض سجلات النشاط',
            ],
            'dashboard' => [
                'view' => 'عرض لوحة التحكم',
            ],
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | سجل النشاط - Activity Log Resource
    |--------------------------------------------------------------------------
    */
    'activities' => [
        'label' => 'نشاط',
        'plural_label' => 'سجل النشاط',
        'navigation_label' => 'سجل النشاط',
        'system_causer' => 'النظام',
        'description_format' => 'تم :event :subject',

        'sections' => [
            'summary' => 'ملخص النشاط',
            'actor' => 'المنفّذ والسجل',
            'changes' => 'الحقول المعدّلة',
            'properties' => 'الخصائص الكاملة',
        ],

        'columns' => [
            'timestamp' => 'الوقت',
            'log_name' => 'السجل',
            'event' => 'الحدث',
            'description' => 'الوصف',
            'subject' => 'السجل المرتبط',
            'subject_id' => 'معرّف السجل',
            'causer' => 'بواسطة',
            'causer_type' => 'نوع المنفّذ',
            'old_values' => 'القيم القديمة',
            'new_values' => 'القيم الجديدة',
            'batch' => 'الدفعة',
        ],

        'events' => [
            'created' => 'إنشاء',
            'updated' => 'تعديل',
            'deleted' => 'حذف',
            'restored' => 'استرجاع',
        ],

        'log_names' => [
            'default' => 'النظام',
        ],

        'values' => [
            'true' => 'نعم',
            'false' => 'لا',
        ],

        /*
        | قاموس عام لأسماء الحقول التي تظهر في جدول "الحقول المعدّلة".
        | يستخدم كاحتياطي عندما لا تغطّي ترجمات الحقول/الأعمدة الخاصة بكل
        | موديل المفتاحَ المطلوب.
        */
        'field_labels' => [
            'name' => 'الاسم',
            'code' => 'الكود',
            'sku' => 'الرقم المرجعي',
            'type' => 'النوع',
            'unit' => 'الوحدة',
            'unit_cost' => 'تكلفة الوحدة',
            'unit_price' => 'سعر الوحدة',
            'quantity' => 'الكمية',
            'received_quantity' => 'الكمية المستلمة',
            'produced_quantity' => 'الكمية المُنتَجة',
            'waste_quantity' => 'كمية الهالك',
            'planned_quantity' => 'الكمية المخططة',
            'waste_percentage' => 'نسبة الهالك %',
            'minimum_stock' => 'الحد الأدنى للمخزون',
            'status' => 'الحالة',
            'priority' => 'الأولوية',
            'version' => 'الإصدار',
            'description' => 'الوصف',
            'notes' => 'ملاحظات',
            'email' => 'البريد الإلكتروني',
            'estimated_budget' => 'الميزانية التقديرية',
            'actual_cost' => 'التكلفة الفعلية',
            'total_amount' => 'المبلغ الإجمالي',
            'on_hand_quantity' => 'الكمية المتاحة',
            'on_hold_quantity' => 'الكمية المحجوزة',
            'warehouse_type' => 'المستودع',
            'reference_type' => 'نوع المرجع',
            'reference_id' => 'معرّف المرجع',
            'file_name' => 'اسم الملف',
            'file_type' => 'نوع الملف',
            'file_size' => 'حجم الملف',
            'category' => 'الفئة',
            'project_id' => 'المشروع',
            'item_id' => 'الصنف',
            'bom_id' => 'قائمة المواد',
            'purchase_order_id' => 'أمر الشراء',
            'approved_by' => 'اعتُمد بواسطة',
            'approved_at' => 'تاريخ الاعتماد',
            'qa_approved_by' => 'اعتُمد الجودة بواسطة',
            'qa_approved_at' => 'تاريخ اعتماد الجودة',
            'created_by' => 'أُنشئ بواسطة',
            'po_number' => 'رقم أمر الشراء',
            'wo_number' => 'رقم أمر التصنيع',
        ],

        'filters' => [
            'date_range' => 'النطاق الزمني',
            'from' => 'من',
            'until' => 'إلى',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | ترجمات الحالات - Enum Translations
    |--------------------------------------------------------------------------
    */
    'enums' => [
        'reservation_status' => [
            'active' => 'نشط',
            'released' => 'محرَّر',
        ],

        'claim_status' => [
            'draft' => 'مسودة',
            'submitted' => 'مُقدَّمة',
            'collected' => 'محصَّلة',
            'cancelled' => 'ملغاة',
        ],

        'payment_direction' => [
            'incoming' => 'وارد (مقبوض)',
            'outgoing' => 'صادر (مدفوع)',
        ],

        'payment_method' => [
            'cash' => 'نقدي',
            'cheque' => 'شيك',
            'bank_transfer' => 'تحويل بنكي',
        ],

        'facility_status' => [
            'active' => 'نشط',
            'expired' => 'منتهٍ',
            'closed' => 'مغلق',
        ],

        'installation_status' => [
            'pending' => 'قيد الانتظار',
            'in_progress' => 'قيد التنفيذ',
            'completed' => 'مكتمل',
        ],

        'bom_status' => [
            'draft' => 'مسودة',
            'pending_approval' => 'بانتظار الاعتماد',
            'approved' => 'معتمد',
            'superseded' => 'مُستبدَل',
        ],

        'project_status' => [
            'draft' => 'مسودة',
            'pending_review' => 'بانتظار المراجعة',
            'approved' => 'معتمد',
            'tender' => 'مناقصة',
            'in_hand' => 'تحت اليد',
            'in_progress' => 'عملية نشطة',
            'on_hold' => 'معلّق',
            'completed' => 'مكتمل',
            'cancelled' => 'ملغي',
            'lost' => 'مفقودة',
        ],

        'arrival_method' => [
            'direct_client' => 'عميل مباشر',
            'consultant_referral' => 'إحالة من استشاري',
            'tender_invitation' => 'دعوة مناقصة',
            'website' => 'الموقع الإلكتروني',
            'other' => 'أخرى',
        ],

        'conductor_type' => [
            'copper' => 'نحاس',
            'aluminum' => 'ألومنيوم',
            'bi_metal' => 'باي ميتال',
        ],

        'attachment_category' => [
            'upload' => 'UPLOAD',
            'vendor_list' => 'VENDOR LIST',
            'drowing' => 'DROWING',
            'speces' => 'SPECES',
            'boq' => 'BOQ',
            'site_measurement' => 'مقاسات الموقع',
            'submittal' => 'Submittal (SMB)',
            'commercial_registry' => 'السجل التجاري',
            'tax_card' => 'البطاقة الضريبية',
            'profit_tax_exemption' => 'مستند إعفاء خصم 1%',
            'po_scan' => 'صورة أمر الشراء',
            'addition_voucher_scan' => 'صورة إذن الإضافة',
            'customer_acceptance' => 'قبول العميل',
            'customer_document' => 'ملفات العميل',
        ],

        'lost_reason' => [
            'high_price' => 'الأسعار غالية',
            'not_approved_by_consultant' => 'غير معتمدين عند الاستشاري',
            'electricity_company_approval' => 'اعتماد شركة الكهرباء',
            'payment_facilities' => 'تسهيلات في الدفع',
            'other' => 'أخرى',
        ],

        'work_order_status' => [
            'draft' => 'مسودة',
            'pending' => 'معلّق',
            'in_progress' => 'قيد التنفيذ',
            'qa_review' => 'مراجعة الجودة',
            'completed' => 'مكتمل',
            'cancelled' => 'ملغي',
        ],

        'quality_sheet_status' => [
            'draft' => 'مسودة',
            'qa_filled' => 'مَلأتها الجودة',
            'approved' => 'معتمدة',
        ],

        'purchase_order_status' => [
            'draft' => 'مسودة',
            'submitted' => 'مُرسَل',
            'partially_received' => 'مستلم جزئياً',
            'received' => 'مستلم',
            'cancelled' => 'ملغي',
        ],

        'item_type' => [
            'raw_material' => 'مادة خام',
            'finished_good' => 'منتج نهائي',
            'semi_finished' => 'نصف مُصنَّع',
            'consumable' => 'مستهلكات',
        ],

        'transaction_type' => [
            'in' => 'وارد',
            'out' => 'صادر',
            'hold' => 'محجوز',
            'release' => 'مُفرَج',
        ],

        'warehouse_type' => [
            'raw_materials' => 'مخزن الخامات',
            'work_in_progress' => 'مخزن تحت التشغيل',
            'finished_goods' => 'مخزن المنتجات التامة',
        ],

        'voucher_status' => [
            'draft' => 'مسودة',
            'posted' => 'مُرحَّل',
        ],

        'loss_type' => [
            'natural' => 'هالك طبيعي',
            'abnormal' => 'هالك غير طبيعي',
        ],

        'invoicing_status' => [
            'not_invoiced' => 'غير مفوتر',
            'partially_invoiced' => 'مفوتر جزئياً',
            'fully_invoiced' => 'مفوتر بالكامل',
        ],

        'purchase_invoicing_status' => [
            'not_invoiced' => 'غير مفوتر (منتظر الفاتورة)',
            'invoiced' => 'مفوتر',
            'closed_uninvoiced' => 'مُقفل بدون فاتورة',
        ],

        'account_direction' => [
            'debit' => 'مدين',
            'credit' => 'دائن',
        ],

        'account_type' => [
            'asset' => 'أصل',
            'liability' => 'التزام',
            'equity' => 'حقوق ملكية',
            'revenue' => 'إيراد',
            'expense' => 'مصروف',
        ],

        // بند القوائم المالية (ماليات.pptx)
        'statement_section' => [
            'cost_of_sales' => 'تكلفة المبيعات (قائمة التشغيل)',
            'sales' => 'المبيعات',
            'sales_returns' => 'مردودات المبيعات',
            'other_revenue' => 'إيرادات متنوعة',
            'fx_differences' => 'فروق العملة',
            'capital_gains' => 'أرباح رأسمالية',
            'general_admin_expenses' => 'مصروفات عمومية',
            'depreciation_expenses' => 'مصروفات وإهلاكات',
            'closed_letters_of_credit' => 'اعتمادات مستندية سبق إقفالها',
            'finance_cost' => 'فوائد مدينة وأعباء تمويلية',
            'fixed_assets' => 'أصول ثابتة',
            'accumulated_depreciation' => 'مجمع الإهلاك',
            'current_assets' => 'أصول متداولة',
            'cash_and_banks' => 'النقدية والبنوك',
            'current_liabilities' => 'التزامات متداولة',
            'provisions' => 'احتياطيات ومخصصات',
            'equity' => 'حقوق الملكية',
            'partners_current_account' => 'جاري شركاء',
            'excluded' => 'خارج المعادلات',
            'memo' => 'حساب نظامي (حاشية)',
        ],

        'document_type' => [
            'payment_order' => 'أمر صرف',
            'supply_receipt' => 'إيصال توريد',
            'settlement' => 'قيد تسوية',
        ],

        'journal_status' => [
            'draft' => 'مسودة',
            'posted' => 'مُرحَّل',
        ],

        'delivery_voucher_status' => [
            'draft' => 'مسودة',
            'pending_approval' => 'بانتظار الاعتماد',
            'active' => 'نشط',
            'cancelled' => 'ملغي',
        ],

        'unit_of_measure' => [
            'pcs' => 'قطعة',
            'kg' => 'كيلوغرام',
            'm' => 'متر',
            'm2' => 'متر مربع',
            'liter' => 'لتر',
            'set' => 'طقم',
            'roll' => 'لفة',
            'sheet' => 'لوح',
            'box' => 'صندوق',
            'ton' => 'طن',
        ],
    ],

    /*
    |--------------------------------------------------------------------------
    | عام - Common / Shared Strings
    |--------------------------------------------------------------------------
    */
    'common' => [
        'no_data' => '—',
        'created_at' => 'تاريخ الإنشاء',
        'auto_generated' => 'تلقائي',
        'action_failed' => 'فشل تنفيذ الإجراء',
        'currency' => 'ج.م',
        'print' => 'طباعة',
        'close' => 'إغلاق',
        // Tooltip for the collapsed row-actions (⋮) dropdown trigger shared
        // by every table.
        'actions' => 'الإجراءات',
    ],
];
